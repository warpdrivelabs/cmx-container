...
| A1.4 | cmx-core 包含必要基础类型 | 对照 `reuse-catalog.md` §1 | `dv!` / `ParamsBuilder` / `DataValue` 全部存在 | 全员存在 | 🟡 警告 |
| A1.5 | 新增 crate 有充分理由 | 人工评估 | 复用性 / 编译隔离 / 团队边界 三者满足其一 | 不为 1 个文件单建 crate | 🟡 警告 |
| A1.6 | 旧 crate 不应留死代码 | 读 `cmx-model/src/dict/`（旧 `data/dict/` 已随资产迁移清理） | 与新接口重复的旧实现应清理或显式标记 | 0 个未标记的重复实现 | 🟡 警告（[AGENTS.md §18](../../../../AGENTS.md) 硬约束） |
| A1.7 | 不应在 cmx-api 中重定义业务 Entity | Grep `pub struct.*Entity` under `crates/libs/cmx-apis/cmx-*-api/src/handlers/` | 业务 Entity 仅在 cmx-biz / cmx-iam 定义 | 0 个重定义 | 🔴 严重（违反职责边界） |
...
| # | 检查项 | 检查方法 | 通过标准 | 量化阈值 | 建议严重级别 |
|---|--------|---------|---------|---------|-------------|
| C1.1 | Error 必派生 `thiserror` | Grep `impl.*Error for` / `impl Display` | 禁止手写 impl | 出现率 0% | 🔴 严重（[AGENTS.md §1.1/1.2](../../../../AGENTS.md)） |
...
| C1.5 | 禁止裸 `unwrap()` | Grep `\.unwrap()`（排除 tests/） | 必用 `?` 或 `expect("有意义提示")` | 散落率 0% | 🟠 中等（[AGENTS.md §1.4](../../../../AGENTS.md)） |
...
| C1.8 | `init()` 返回 `Result<()>` | 读 `init()` 函数 | 禁止 `panic!` / `expect` / `unwrap` | 违例 0 | 🔴 严重（[AGENTS.md §17](../../../../AGENTS.md)） |
...
| C3.2 | **API 文档覆盖率** | 读 `pub fn` | 每个 `pub fn` 必含 `# Arguments` + `# Returns` + `# Examples` | 覆盖率 100% | 🟠 中等（[AGENTS.md §13](../../../../AGENTS.md)） |
...
| C3.19 | **无 dead_code** | `cargo build` 输出 | 不用代码删或 `#[allow(dead_code)]` 注释保留 | 活跃 0 | 🟡 警告（[AGENTS.md §3.6](../../../../AGENTS.md)） |
...
| # | 检查项 | 检查方法 | 通过标准 | 量化阈值 | 建议严重级别 |
|---|--------|---------|---------|---------|-------------|
| D1.1 | workspace 集中管理依赖 | 读子 `Cargo.toml` | 子 crate 无 `version = "x.y"` 形式 | 违例 0 | 🔴 严重（[AGENTS.md §3.1](../../../../AGENTS.md)） |
| D1.2 | 用 `workspace = true` | Grep `{ workspace = true }` | 必有 | 缺失 0 | 🔴 严重 |
| D1.3 | 依赖有清晰注释 | 读 `Cargo.toml` | 每个依赖上方一行注释 | 缺失 0 | 🟡 警告（[AGENTS.md §3.3](../../../../AGENTS.md)） |
| D1.4 | 禁止分组注释 | Grep `^# ===` | 单行注释 | 违例 0 | 🔵 建议 |
| D1.5 | 禁止 `log` crate | Grep `^log = \| use log::` | 必用 `tracing` | 出现率 0% | 🔴 严重（[AGENTS.md §3.4](../../../../AGENTS.md)） |
| D1.6 | 未用依赖注释保留 | 读 `Cargo.toml` | 不删而注释 `# [dependencies]\n# xxx = "1.0"` | 缺失 0 | 🔵 建议（[AGENTS.md §3.6](../../../../AGENTS.md)） |
...
| D2.7 | `plugin_id` 用下划线 | Grep `plugin_id.*-` | 禁止连字符 | 违例 0 | 🟡 警告（[AGENTS.md §11](../../../../AGENTS.md)） |
| D2.8 | 表名以 `cmx_` 前缀 | 读 DDL | 系统表必带 | 违例 0 | 🟡 警告（[AGENTS.md §5.4](../../../../AGENTS.md)） |
| D2.9 | 禁止外键约束 | Grep `FOREIGN KEY` | DDL 必无 | 出现 0 | 🟡 警告（[AGENTS.md §5.4](../../../../AGENTS.md)） |
...
| # | 检查项 | 检查方法 | 通过标准 | 量化阈值 | 建议严重级别 |
|---|--------|---------|---------|---------|-------------|
| D3.1 | `pub fn` 含 `# Arguments` + `# Returns` | Grep `pub fn` 上方 | 必含 | 覆盖率 100% | 🟠 中等（[AGENTS.md §13](../../../../AGENTS.md)） |
...
| D3.8 | `#[plugin_fn]` 函数摘要**不以句号结尾** | 读 WASM 函数 | 例外（[AGENTS.md §13](../../../../AGENTS.md)） | 例外豁免 | — |
...
|---------|------|---------|---------|---------|
| `app_id` 取自 `get_app_id()`，禁止硬编码 `"default"` | [AGENTS.md §6](../../../../AGENTS.md) | Grep `"default"` 在 plugin install/import | 出现 0 | 🔴 严重 |
| `app_id ≡ module_code`（当前架构），不要把 `application_code` 当 `app_id` | [AGENTS.md §6](../../../../AGENTS.md) | 读 install/import 逻辑 | 误用 0 | 🟡 警告 |
| Service `list/page` 必用 `filters + list_options` | [AGENTS.md §7](../../../../AGENTS.md) | Grep `(page_size, keyword,` 散落 | 散落 0 | 🟠 中等 |
| Handler 路由除 `get_by_id` 外全 POST + JSON | [AGENTS.md §8.3](../../../../AGENTS.md) | Grep `.route(.*get(` | 误用 0 | 🟠 中等 |
| `ForCreate` 不含 id/create_time/update_time | [AGENTS.md §8.3](../../../../AGENTS.md) | 读 Entity 定义 | 违例 0 | 🟡 警告 |
| `ForUpdate` 字段全 Option | [AGENTS.md §8.3](../../../../AGENTS.md) | 读 Entity 定义 | 违例 0 | 🟡 警告 |
| `Entity` 必 `derive(Fields)` | [AGENTS.md §9](../../../../AGENTS.md) | Grep `derive.*Fields` | 缺失 0 | 🔴 严重 |
| `Filter` 必 `derive(FilterNodes)`，字段用 `OpValsXxx` | [AGENTS.md §9](../../../../AGENTS.md) | Grep `FilterNodes` | 缺失 0 | 🔴 严重 |
| 必用 `execute_sql_with_datavalues` 而非 `_with_json` | [AGENTS.md §10](../../../../AGENTS.md) | Grep `_with_json` | 出现 0 | 🔴 严重 |
| 动态 UPDATE 必用 `ParamsBuilder` | [AGENTS.md §10](../../../../AGENTS.md) | Grep `format!("\$\d` | 散落 0 | 🟠 中等 |
| 事务内传 `txn_id: Some(&txn_id)` | [AGENTS.md §10](../../../../AGENTS.md) | 读事务代码 | 误用 0 | 🟡 警告 |
| `plugin_id` 只能用 `_` | [AGENTS.md §11](../../../../AGENTS.md) | Grep `plugin_id.*-` | 违例 0 | 🟠 中等 |
| 插件 metadata `ordinal` 连续不跳跃 | [AGENTS.md §12](../../../../AGENTS.md) | 读 metadata | 跳跃 0 | 🟡 警告 |
| 种子数据 `conflict_columns` 选业务唯一列 | [AGENTS.md §12](../../../../AGENTS.md) | 读 seeddata | 误选 0 | 🟡 警告 |
| cmx-core 不引入业务依赖 | [AGENTS.md §14](../../../../AGENTS.md) | 读 `cmx-core/Cargo.toml` | 0 业务依赖 | 🔴 严重 |
| 迁移文件 `YYYYMMDD_XXX.{up,down}.sql` | [AGENTS.md §5.5](../../../../AGENTS.md) | `ls docs/sql/migrations/` | 命名违例 0 | 🟡 警告 |
| `init_ddl.sql` 保持最新完整 | [AGENTS.md §5.6](../../../../AGENTS.md) | 读迁移文件 | 同步 0 偏差 | 🟡 警告 |
| 旧接口标记（cmx-model/src/dict/ 等）不参考 | [AGENTS.md §18](../../../../AGENTS.md) | 读新代码 | 参考 0 | 🔴 严重 |
