...
**位置**：[`crates/libs/cmx-core/src/`](../../../../crates/libs/cmx-core/src)

| 资产 | 路径 / 锚点 | 用途 | 适用场景 | 误用反例 |
|------|------------|------|---------|----------|
| **`dv!` 宏** | `model/cell.rs` | 批量构造 `Vec<DataValue>`，自动类型推断 | SQL 参数构造 | ❌ `vec![id.into(), name.into()]` 链式 `.into()`（冗长且丢类型） |
...
**位置**：[`crates/libs/cmx-utils/src/`](../../../../crates/libs/cmx-utils/src)
...
**位置**：[`crates/libs/cmx-infra/cmx-database/src/`](../../../../crates/libs/cmx-infra/cmx-database/src)

| 资产 | 路径 | 用途 | 适用场景 | 误用反例 |
|------|------|------|---------|----------|
| **`DatabaseManager`** | `manager/` | 数据库连接池 + 事务管理 | 任何 SQL 执行 | ❌ 直接创建 `PgPool` |
| **`GenericCrudService<MC>`** | `crud/crud_fns.rs` | 通用 CRUD（create/get/update/delete/list/page） | Service 层基础操作 | ❌ 手写 CRUD SQL |
| **`DbBmc` trait** | `crud/mod.rs` | BMC 必须实现 | 每个实体表对应一个 BMC | ❌ BMC 缺失或字段不全 |
...
**位置**：[`crates/libs/cmx-infra/cmx-database-pg/src/`](../../../../crates/libs/cmx-infra/cmx-database-pg/src)

> **核心定位**：cmx-database（sqlx 栈）与 cmx-database-pg（tokio-postgres 栈）**双栈并存**，经 `cmx-service-base::register_pg_datasources` 从同一组 `[[databases]]` 注册（双栈注册），API 高度对齐。
>
> **选择规则**：新代码默认用 cmx-database；仅在需要下方 4 项独有能力时引入 cmx-database-pg。

### 独有资产（cmx-database 没有的）

| # | 资产 | 路径 | 用途 | 误用反例 |
|---|------|------|------|----------|
| ① | **`query_sql_zmc_stream_chunks`** | `manager/mod.rs` + `connection/mod.rs` | 真·分帧流式：基于 `mpsc::Sender<Bytes>`，逐行编码为长度分帧发送，峰值内存 O(单行)，16KB 攒批刷写，header 帧先发、空结果容错 | ❌ 小结果集用此（无收益，徒增依赖） |
| ② | **数组列读取还原** | `executor/mod.rs`（`PgResultConverter::convert_rows`） | 读取阶段支持 TEXT_ARRAY / INT8_ARRAY / UUID_ARRAY -> `DataValue::Array`。cmx-database 读取方向**不还原数组**（只在绑定时支持写入） | ❌ 误以为 cmx-database 也能读取数组列 |
| ③ | **`get_conn()`** | `connection/mod.rs` | 返回 `deadpool_postgres::Object`，供事务层跨 await 手动驱动 BEGIN/COMMIT | ❌ cmx-database 用 sqlx 的 `pool.begin()`，无需此方法 |
| ④ | **4 个 ToSql 适配器** | `executor/mod.rs` | `PgInt` / `PgDateTime` / `PgDateTimeNull` / `PgIntNull`。tokio-postgres 类型校验严格（i64 绑 INT4 列会 WrongType），需宽度/时区自适应包装 | ❌ 这是 tokio-postgres 驱动刚需，sqlx 隐式协调不需要 |
...
🔴 **不能简单替换** 的场景（需迁移实现）：
- 依赖 `query_sql_zmc_stream_chunks`（O(单行) 内存流式；当前仅 `crates/tests/cmx-database-test` 的 `mem_bench.rs` / `e2e_server.rs` 与 pg 自身在用）
- 依赖数组列读取还原（`DataValue::Array` 从数据库读取）
- 直接依赖 `TokioPgRowSource` 全路径（如 `cmx-database-test` 的 `e2e_server.rs`、`mem_bench.rs`）需改为 `SqlxPgRowSource`
...
**位置**：[`crates/libs/cmx-traits/src/`](../../../../crates/libs/cmx-traits/src)
...
**位置**：[`crates/libs/cmx-biz/src/`](../../../../crates/libs/cmx-biz/src)

| 资产 | 路径 | 用途 | 适用场景 | 误用反例 |
|------|------|------|---------|----------|
| **7 大业务模块** | `application/` `datasource/` `domain/` `form/` `menu/` `module/` `validation/`（另有 `errcode/` 错误码） | 业务实体的 Entity/BMC/Filter/Service 标准四件套 | 任何业务实体接入 | ❌ 在 cmx-apis 域 crate 中重定义 Entity |
| **`ResourceDataImporterImpl`** | `resource_importer.rs` | 多类别资源数据导入（Form/Menu/Perm） | 资源数据导入 | ❌ 手写 JSON 解析 + 单类实现 |
| **`function_invoker`** | `function_invoker.rs` | 插件函数调用核心逻辑 | 协议无关的函数调用 | ❌ 在 handler 中手写函数调用流程 |
| **`service_executor`** | `service_executor.rs` | 服务编排执行核心逻辑 | 协议无关的编排 | ❌ 在 handler 中手写编排执行 |
...
```bash
# cmx-apis 域 crate 中不应手写 SQL（应走 cmx-biz Service）
grep -rn "execute_sql\|query_sql" crates/libs/cmx-apis/cmx-*-api/src/handlers/

# cmx-apis 域 crate 中不应重定义 Entity
grep -rn "pub struct.*Entity" crates/libs/cmx-apis/cmx-*-api/src/handlers/
```

---

## 六、cmx-apis（HTTP 适配层 + 通用 Handler 框架，原 cmx-api 拆分）

**位置**：[`crates/libs/cmx-apis/`](../../../../crates/libs/cmx-apis)（cmx-api-core + cmx-api-types + 6 个域 crate：cmx-biz-api / cmx-common-api / cmx-iam-api / cmx-plugin-api / cmx-ai-api / cmx-storage-api）

| 资产 | 路径 | 用途 | 适用场景 | 误用反例 |
|------|------|------|---------|----------|
| **`declare_crud_handlers!` 宏** | `cmx-api-core/src/routes/macros.rs` | 一行声明 8 个 CRUD handler | 标准实体 Handler | ❌ 手写 8 个 `async fn` |
| **`register_crud_routes!` 宏** | `cmx-api-core/src/routes/macros.rs` | 路由注册（旧版，无 OpenAPI） | 简单内部 API | ❌ 散落 `.route("/create", post(...))` |
| **8 个通用 Handler** | `cmx-api-core/src/rest/handler.rs` | `create` / `get_by_id` / `update` / `delete` / `list` / `page` 等 | 任何 CRUD 路由 | ❌ 重新实现通用 CRUD 逻辑 |
| **`CmxAppState`** | `cmx-api-core/src/app_state.rs` | 全局应用状态 | 注入业务 Service | ❌ 手写 `Arc<Mutex<...>>` |
| **`ApiResp<T>` / `Pagination` / `UnitResp`** | `cmx-api-types`（域 crate re-export） | 统一响应格式 | Handler 返回值 | ❌ 自定义 `Json<MyResp<T>>` |
| **`Error` / `ErrCode` / `Result`** | `cmx-api-types`（域 crate re-export） | 统一错误 | Handler 错误 | ❌ `anyhow::Error` 直接返回 |
| **`ModuleRoutes` trait** | `cmx-api-core/src/routes/traits.rs` | 路由模块化注册 | 多模块路由 | ❌ 在 `main.rs` 中散落注册 |
| **域 Handler 落位** | `cmx-apis/cmx-<domain>-api/src/handlers/`（如 `cmx-iam-api/src/handlers/iam/user/handler.rs`、`cmx-common-api/src/handlers/portal/`） | 各业务域 handler 归属 | 新增 handler 时选对 crate | ❌ 把新 handler 塞回 core 或放错域 |
| **OpenAPI 注解工具** | `cmx-api-core/src/routes/macros.rs` + 各域 `openapi.rs` | utoipa 文档生成 | API 文档 | ❌ Handler 缺 `#[utoipa::path]` |

**审查锚点**：

```bash
# 应使用 declare_crud_handlers! 宏而非手写
grep -rn "async fn create\b\|async fn list\b" crates/libs/cmx-apis/cmx-*-api/src/handlers/

# 应使用统一响应类型
grep -rn "Json<.*>" crates/libs/cmx-apis/cmx-*-api/src/handlers/  # 初筛自定义响应

# 应使用 ModuleRoutes trait
grep -rn "impl.*Router\b" crates/libs/cmx-apis/cmx-*-api/src/handlers/  # 初筛
```
...
**位置**：[`crates/libs/cmx-macros/src/`](../../../../crates/libs/cmx-macros/src)
...
```bash
# 应使用属性宏而非函数体手写
grep -rn "require_permission\|require_role" crates/libs/cmx-apis/cmx-*-api/src/handlers/  # 初筛
```
...
**位置**：[`crates/libs/cmx-iam/src/`](../../../../crates/libs/cmx-iam/src)
...
**位置**：[`crates/libs/cmx-plugin/src/`](../../../../crates/libs/cmx-plugin/src)
...
## 十、cmx-form / cmx-common-api / cmx-metadata（其他业务域）

| 资产 crate | 关键导出 | 用途 | 误用反例 |
|-----------|---------|------|----------|
| **cmx-form** | `pages::form::Form` / `pages::html::HtmlPage` / `pages::native::NativePage` | 三种表单页面渲染 | ❌ 在 cmx-apis 域 crate 中直接拼 HTML |
| **cmx-common-api**（原 cmx-portal 并入） | `handlers/portal/`（portal 菜单/工作区代理）、`handlers/service/` 等 | 门户/公共业务 handler | ❌ 把门户逻辑写进其他域 crate |
| **cmx-metadata** | `ddl::*` / `parser::*` / `seed::*` / `executor::*` | 表元数据 DDL 生成/解析/执行 | ❌ 手写 DDL 字符串 |
| **cmx-model** | `dict::*` | 字典元数据建模 | ❌ 在 cmx-apis 域 crate 中手写字典解析 |

> 注：DCT/DOC 的 `/api/dct/*`、`/api/doc/*` handler 已迁独立仓 `cmx-model`（模型中心微服务），cmx-container 内不再有 dct.rs / doc.rs。
...
- 项目总规范：[AGENTS.md 19 章](../../../../AGENTS.md)
