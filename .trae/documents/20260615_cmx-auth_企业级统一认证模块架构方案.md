...
crates/libs/cmx-infra/cmx-auth/
  ...
  │   ├── session/                  # 会话管理
│   │   ├── mod.rs
│   │   ├── manager.rs            # SessionManager (创建/查询/销毁/互踢/心跳续期)
...
> **评审 P0-4 + 5.1 修正**：遵循 cmx-traits 扁平结构，新增独立文件（位于 `crates/libs/cmx-traits/src/auth/` 子目录）

```
crates/libs/cmx-traits/src/
  ├── auth/
  │   ├── mod.rs                # 认证模块入口，re-export 所有 trait
  │   ├── error.rs              # AuthError 定义
  │   ├── service.rs            # AuthService trait + Credentials + TokenPair + DeviceInfo
  │   ├── policy.rs             # AuthPolicy trait
  │   ├── storage_query.rs      # AuthStorageQuery trait（Auth 表操作）
  │   └── user_query.rs         # 🆕 用户认证数据查询抽象（解耦 cmx-auth 与 cmx-iam）
  └── ... 其他模块
```
...
///
/// 字段由 cmx-iam 的 cmx_user 表映射（不含 password_hash 之外的扩展列）。
...
/// 实现 crates/libs/cmx-iam/src/user_auth_query_impl.rs
#[async_trait]
...
```rust
// crates/libs/cmx-traits/src/auth/error.rs
use thiserror::Error;
...
#### 4.4.2 Token 事件流表（append-only 事件流）

```sql
CREATE TABLE "public"."cmx_auth_token_event" (
    "id" varchar(64) NOT NULL,
...
CREATE INDEX "idx_cmx_auth_token_event_user" ON "public"."cmx_auth_token_event" ("user_id");
CREATE INDEX "idx_cmx_auth_token_event_type" ON "public"."cmx_auth_token_event" ("event_type");

COMMENT ON TABLE "public"."cmx_auth_token_event" IS 'Token 事件流表（append-only，记录签发/撤销/刷新/登录成功/登录失败/密码修改等事件）';
COMMENT ON COLUMN "public"."cmx_auth_token_event"."event_type" IS '事件类型：token_issued/token_revoked/token_refreshed/login_success/login_failed/password_changed';
COMMENT ON COLUMN "public"."cmx_auth_token_event"."user_id" IS '用户ID';
COMMENT ON COLUMN "public"."cmx_auth_token_event"."jti" IS 'JWT ID（关联具体 Token，部分事件可能无 jti）';
COMMENT ON COLUMN "public"."cmx_auth_token_event"."detail" IS '事件详情（JSON，如 IP/设备/失败原因等）';
```

**生命周期管理**（append-only 事件流模式）：

* 每次事件（签发/撤销/刷新/登录成功/登录失败/密码修改）INSERT 一条记录，不 UPDATE

* 定时任务每天凌晨清理 30 天前的事件记录

* 保留 30 天用于安全审计和问题排查

* `UPDATE cmx_auth_token_event SET archived = 1 WHERE create_time < NOW() - INTERVAL '30 days'`
...
| Token Event 保留独立事件流表 + 定期清理          | append-only 事件流，记录签发/撤销/刷新等事件，30 天后归档 |
