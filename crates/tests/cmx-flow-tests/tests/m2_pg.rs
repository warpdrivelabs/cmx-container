...
async fn pg_parallel_fork_join_and_history() {
    ...
    // 启动 → fork 出两个并行任务。
    let started = engine
        .start_process("pg_parallel_sign", Variables::new(), Some("PG-PAR-001".into()))
        .await
        .expect("启动应成功");
    ...
    let snap_mid = engine.store().load_snapshot(&started.instance_id).await.unwrap();
    assert_eq!(
        snap_mid.tokens.iter().filter(|t| t.state == TokenState::Joining).count(),
        1,
        "财务分支应作为 JOINING 令牌落库"
    );

    // 办结法务 → 合流 → 完成。
    let legal = mid.open_tasks.iter().find(|t| t.node_bpmn_id == "legal").unwrap().clone();
    let done = engine
    ...
    let snap_final = engine.store().load_snapshot(&started.instance_id).await.unwrap();
    ...
    // 清理，保持可重复运行。
    for sql in [
        format!("DELETE FROM cmx_flow_hi_task WHERE instance_id = '{}'", started.instance_id),
        format!("DELETE FROM cmx_flow_hi_instance WHERE id = '{}'", started.instance_id),
        format!("DELETE FROM cmx_flow_task WHERE instance_id = '{}'", started.instance_id),
        format!("DELETE FROM cmx_flow_token WHERE instance_id = '{}'", started.instance_id),
        format!("DELETE FROM cmx_flow_instance WHERE id = '{}'", started.instance_id),
    ] {
