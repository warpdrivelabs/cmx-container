...
async fn start() -> (Engine<InMemoryStore>, InMemoryStore, String, String) {
    ...
    let s = engine.start_process("approve", Variables::new(), None).await.unwrap();
...
async fn transfer_swaps_person_completely() {
    ...
    engine.transfer_task(&iid, &task, "张三", "李四", Some("我出差")).await.unwrap();
    ...
    assert_eq!(t.owner_user_id.as_deref(), Some("李四"), "转办彻底换人，owner 也归李四");
    assert_eq!(snap.delegations.len(), 1);
    assert_eq!(snap.delegations[0].kind, "TRANSFER");

    // 李四可正常办结 → 完成。
    let done = engine.complete_task(&iid, &task, Variables::new()).await.unwrap();
...
async fn delegate_keeps_owner() {
    let (engine, store, iid, task) = start().await;
    engine.delegate_task(&iid, &task, "张三", "李四", Some("帮我看下")).await.unwrap();

    let snap = store.load_snapshot(&iid).await.unwrap();
    let t = snap.tasks.iter().find(|t| t.id == task).unwrap();
    assert_eq!(t.assignee.as_deref(), Some("李四"), "代理人=李四");
    assert_eq!(t.owner_user_id.as_deref(), Some("张三"), "委派 owner 仍是张三");
...
async fn add_sign_before_inserts_temp_then_resumes() {
    let (engine, store, iid, task) = start().await;
    // 向前加签：先让王五审。
    let after = engine.add_sign(&iid, &task, "张三", "王五", true, Some("请先过目")).await.unwrap();
    // 现在有两个未办结任务：原任务(挂起) + 临时任务(王五)。
    let snap = store.load_snapshot(&iid).await.unwrap();
    let orig = snap.tasks.iter().find(|t| t.id == task).unwrap();
    assert_eq!(orig.delegation_state.as_deref(), Some("SUSPENDED"), "原任务挂起");
    let temp = snap.tasks.iter().find(|t| t.parent_task_id.as_deref() == Some(task.as_str())).unwrap();
    ...
    // 挂起守卫：原任务不可直接办结。
    assert!(engine.complete_task(&iid, &task, Variables::new()).await.is_err(), "挂起任务不可办结");

    // 王五办结临时任务 → 原任务恢复（不推进流程）。
    let mid = engine.complete_task(&iid, &temp_id, Variables::new()).await.unwrap();
    ...
    assert_eq!(snap2.tasks.iter().filter(|t| !t.completed).count(), 1, "只剩原任务待办");

    // 张三办结原任务 → 完成。
    let done = engine.complete_task(&iid, &task, Variables::new()).await.unwrap();
    assert_eq!(done.state, InstanceState::Completed);
    assert_eq!(store.load_snapshot(&iid).await.unwrap().delegations[0].kind, "ADDSIGN_BEFORE");
}

#[tokio::test]
async fn add_sign_after_recorded_and_resumes() {
    let (engine, store, iid, task) = start().await;
    engine.add_sign(&iid, &task, "张三", "王五", false, None).await.unwrap();
    let snap = store.load_snapshot(&iid).await.unwrap();
    ...
    // 临时办结 → 恢复 → 原任务办结 → 完成。
    engine.complete_task(&iid, &temp_id, Variables::new()).await.unwrap();
    let done = engine.complete_task(&iid, &task, Variables::new()).await.unwrap();
    assert_eq!(done.state, InstanceState::Completed);
}
...
async fn nested_add_sign() {
    let (engine, store, iid, task) = start().await;
    // 张三加签王五。
    engine.add_sign(&iid, &task, "张三", "王五", true, None).await.unwrap();
    let temp1 = store.load_snapshot(&iid).await.unwrap()
        .tasks.iter().find(|t| t.parent_task_id.as_deref()==Some(task.as_str())).unwrap().id.clone();
    // 王五再加签赵六（临时任务嵌套加签）。
    engine.add_sign(&iid, &temp1, "王五", "赵六", true, None).await.unwrap();

    let snap = store.load_snapshot(&iid).await.unwrap();
    // temp1 挂起，temp2(赵六) 待办，原任务仍挂起。
    let temp1_t = snap.tasks.iter().find(|t| t.id == temp1).unwrap();
    assert_eq!(temp1_t.delegation_state.as_deref(), Some("SUSPENDED"), "temp1 被嵌套加签而挂起");
    let temp2 = snap.tasks.iter().find(|t| t.parent_task_id.as_deref()==Some(temp1.as_str())).unwrap().id.clone();
    assert_eq!(snap.tasks.iter().filter(|t| !t.completed).count(), 3, "原+temp1+temp2 三个未办结");
    assert_eq!(snap.delegations.len(), 2);

    // 赵六办结 temp2 → temp1 恢复。
    engine.complete_task(&iid, &temp2, Variables::new()).await.unwrap();
    let s2 = store.load_snapshot(&iid).await.unwrap();
    assert!(s2.tasks.iter().find(|t| t.id==temp1).unwrap().delegation_state.is_none(), "temp1 恢复");
    // 王五办结 temp1 → 原任务恢复。
    engine.complete_task(&iid, &temp1, Variables::new()).await.unwrap();
    let s3 = store.load_snapshot(&iid).await.unwrap();
    assert!(s3.tasks.iter().find(|t| t.id==task).unwrap().delegation_state.is_none(), "原任务恢复");
    // 张三办结 → 完成。
    let done = engine.complete_task(&iid, &task, Variables::new()).await.unwrap();
    assert_eq!(done.state, InstanceState::Completed);
}
