...
- `handlers/` 中的业务逻辑通过泛型 `H: HostFunctions` 依赖抽象，不直接依赖 Extism PDK
- 测试时使用 `MockHostFunctions`（由 `mockall` 自动生成），无需真实宿主环境
- `extism/` 仅负责将 `PluginCore` 的方法暴露为 Extism 插件函数，是薄适配层
- 按业务实体拆分文件，每个实体文件包含该实体的全部操作（CRUD、缓存、业务逻辑等）
...
在 `src/models/` 目录中定义函数的输入/输出结构体。通用模型放在 `common.rs`，业务实体模型放在对应的 `{entity}.rs` 文件中：
...
> 新建实体文件后，需在 `src/models/mod.rs` 中添加 `pub mod {entity};` 声明。

### 步骤二：实现核心逻辑

在 `src/handlers/` 目录的对应实体文件中，为 `PluginCore` 添加业务方法：

```rust
// src/handlers/{entity}.rs
...
> 新建实体文件后，需在 `src/handlers/mod.rs` 中添加 `pub mod {entity};` 声明。

### 步骤三：暴露插件函数

在 `src/extism/` 目录的对应实体文件中添加 Extism 入口函数：

```rust
// src/extism/{entity}.rs
#[plugin_fn]
...
> 新建实体文件后，需在 `src/extism/mod.rs` 中添加 `pub mod {entity};` 声明。

### 步骤四：编写单元测试

在 `src/tests/` 目录的对应实体文件中添加 mock 测试：
...
> 新建实体文件后，需在 `src/tests/mod.rs` 中添加 `pub mod {entity};` 声明。
...
| 方法 | 说明 |
|------|------|
| `host.call_plugin(request: PluginFunRequest)` | 调用本插件函数 |
| `host.call_remote_plugin(request: PluginFunRequest)` | 调用远程插件函数 |

`PluginFunRequest` 字段：

```rust
PluginFunRequest {
    plugin_id: "target_plugin".to_string(),     // 目标插件 ID（下划线命名）
...
| 方法 | 说明 |
|------|------|
| `host.call_service_by_key(request: CallServiceRequest)` | 调用本服务编排 |
| `host.call_remote_service(request: CallServiceRequest)` | 调用远程服务编排 |
...
| `plugin.id` | 插件唯一标识（**只能使用下划线 `_` 分隔，禁止连字符 `-`**，如 `cmx_account`） |
| `plugin.name` | 插件显示名称 |
...
由于模板采用三层分离架构，`handlers/` 中的业务逻辑与 Extism PDK 完全解耦，因此推荐通过**单元测试 + Mock** 进行调试，这是最高效的方式。
...
let request = PluginFunRequest {
    plugin_id: "target_plugin".to_string(),     // 下划线命名
...
1. **新增函数必须同时修改对应的模块文件**：
   - `models/` — 在对应实体文件中添加模型（或新建实体文件并在 mod.rs 中注册）
   - `handlers/` — 在对应实体文件中添加业务逻辑（或新建实体文件并在 mod.rs 中注册）
   - `extism/` — 在对应实体文件中添加 `#[plugin_fn]` 入口（或新建实体文件并在 mod.rs 中注册）
   - `tests/` — 在对应实体文件中添加单元测试
2. **业务逻辑只写在 `handlers/` 中**，不要在 `extism/` 中写业务代码
...
8. **plugin_id 命名约束**：只能使用下划线 `_` 分隔，禁止使用连字符 `-`（如 `test_plugin`，不能写成 `test-plugin`）
