...
fn assert_page_id(id: &str) -> PortalResult<String> {
    ...
    if !is_safe_id(t) {
        return Err(PortalError::bad_request("页面 ID 仅允许字母、数字、._-，长度 1–128"));
    }
    Ok(t.to_string())
}
...
pub fn parse_page_namespace(id: &str) -> PortalResult<PageNamespace> {
    ...
    for s in &segs {
        if s.is_empty() {
            return Err(PortalError::bad_request("页面 ID 段不能为空（禁止前导/尾随点或连续点）"));
        }
        if !is_safe_segment(s) {
            return Err(PortalError::bad_request(format!("页面 ID 段非法：\"{s}\"（仅允许字母、数字、_-）")));
        }
    }
    if segs.len() == 1 {
    ...
    Ok(PageNamespace { id: clean, domain, app, module, page, rel_path, is_legacy })
}
...
async fn persist_shard(domain: &str, pages: &[serde_json::Value]) -> PortalResult<()> {
    write_json_atomic(&shard_path(domain), &json!({ "version": 1, "domain": domain, "pages": pages }), true).await
}
...
async fn persist_top_domains(domains: &[String]) -> PortalResult<()> {
    ...
    set.sort();
    set.dedup();
    write_json_atomic(&top_index_path(), &json!({ "version": 2, "domains": set }), true).await
}

/// 防穿越：relPath 不得绝对 / 含 `..` / 反斜杠。
fn safe_rel(rel: &str) -> PortalResult<String> {
    if rel.starts_with('/') || rel.contains("..") || rel.contains('\\') {
        return Err(PortalError::bad_request("relPath 非法（含 ..、\\ 或前导 /）"));
    }
    Ok(rel.to_string())
}
...
fn resolve_html_abs(row: &serde_json::Value) -> PortalResult<std::path::PathBuf> {
    if let Some(rel) = row.get("relPath").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
        let safe = safe_rel(rel)?;
        return Ok(source_abs(&safe));
    }
    if let Some(latest) = row.get("latestHtmlFile").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
    ...
    if id.is_empty() {
        return Err(PortalError::business("页面记录缺少 id / relPath / latestHtmlFile"));
    }
...
async fn find_row_anywhere(id: &str) -> PortalResult<Option<serde_json::Value>> {
    if let Ok(ns) = parse_page_namespace(id) {
        ...
        if let Some(r) = shard.into_iter().find(|r| r.get("id").and_then(|v| v.as_str()) == Some(id)) {
            return Ok(Some(r));
        }
    }
    let list = load_list().await?;
    Ok(list.into_iter().find(|r| r.get("id").and_then(|v| v.as_str()) == Some(id)))
}
...
async fn read_full_from_row(row: &serde_json::Value) -> PortalResult<serde_json::Value> {
    ...
    if html.is_none() {
        ...
        if row.get("relPath").is_some()
            && let Some(latest) = row.get("latestHtmlFile").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
                let base = std::path::Path::new(latest)
                    .file_name()
                    .map(|s| s.to_string_lossy().to_string())
                    .unwrap_or_default();
...
pub async fn list_html_pages_paged(
    ...
    let filtered: Vec<&serde_json::Value> = pages
        ...
        .filter(|r| {
            if !fd.is_empty() {
                let rd = r.get("domain").and_then(|v| v.as_str()).filter(|s| !s.is_empty()).unwrap_or("_legacy");
                if rd != fd {
                    return false;
                }
            }
...
pub async fn save_html_page(input: HtmlPageInput) -> PortalResult<serde_json::Value> {
    ...
    let html = input.html.ok_or_else(|| PortalError::bad_request("html 必须为字符串"))?;
    let ns = parse_page_namespace(&id)?;
    let domain = input.domain.map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).unwrap_or_else(|| ns.domain.clone());
    let app = input.app.map(|s| s.trim().to_string()).unwrap_or_else(|| ns.app.clone());
    let module = input.module.map(|s| s.trim().to_string()).unwrap_or_else(|| ns.module.clone());
    ...
    if let Some(existing) = shard.iter_mut().find(|r| r.get("id").and_then(|v| v.as_str()) == Some(id.as_str())) {
        *existing = row.clone();
    } else {
    ...
    if let Some(existing) = list.iter_mut().find(|r| r.get("id").and_then(|v| v.as_str()) == Some(id.as_str())) {
...
pub async fn get_html_page_by_id(id: &str) -> PortalResult<serde_json::Value> {
    ...
    let row = find_row_anywhere(&pid).await?.ok_or_else(|| PortalError::not_found("页面不存在"))?;
...
pub async fn get_html_pages_by_ids(body: &serde_json::Value) -> PortalResult<serde_json::Value> {
    let ids: Vec<String> = if let Some(arr) = body.as_array() {
        ...
        return Err(PortalError::bad_request("请求体须为 { ids: string[] } 或 JSON 字符串数组"));
    };
    ...
    if cleaned.len() > MAX_BATCH {
        return Err(PortalError::bad_request(format!("单次最多 {MAX_BATCH} 个页面 ID")));
    }
    ...
    let mut shard_cache: std::collections::HashMap<String, Vec<serde_json::Value>> = std::collections::HashMap::new();
    ...
    for (domain, id_list) in by_domain {
        ...
        for id in id_list {
            let mut row = shard.iter().find(|r| r.get("id").and_then(|v| v.as_str()) == Some(id.as_str())).cloned();
            if row.is_none() {
                ...
                row = legacy_list
                    .as_ref()
                    .and_then(|l| l.iter().find(|r| r.get("id").and_then(|v| v.as_str()) == Some(id.as_str())).cloned());
            }
            match row {
