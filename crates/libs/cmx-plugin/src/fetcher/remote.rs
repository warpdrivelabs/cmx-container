...
impl RemoteFetcher {
    ...
    /// 带重试的下载文件
    async fn download_file_with_retry(&self, url: &str, target: &Path) -> Result<(), String> {
        ...
        Err(format!("下载失败，已重试 {} 次: {}", self.max_retries, last_error))
    }
    
    /// 下载文件
    /// 
    /// 使用 HTTP 客户端下载文件。
    async fn download_file(&self, url: &str, target: &Path) -> Result<(), String> {
    ...
    /// 
    /// 验证下载文件的 SHA256 校验和。
    fn verify_sha256(&self, file_path: &Path, expected: &str) -> Result<(), String> {
        use std::io::Read;
        ...
        if actual != expected.to_lowercase() {
            return Err(format!("SHA256校验和不匹配: 期望 {}, 实际 {}", expected, actual));
        }
    ...
    pub async fn get_file_info(&self, url: &str) -> Result<RemoteFileInfo, String> {
        let client = reqwest::Client::builder()
