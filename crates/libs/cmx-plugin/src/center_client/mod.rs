...
//!
//! 本模块保留仍在使用的部分:
//! - [`config`]: `CenterClientConfig`(`[center_client.services]` 单表,定位(url/discovery)与
//!   传输(transport,仅服务间调用生效)均按服务键独立配置,新增微服务只在 toml 加一行,
//!   配置层零代码改动)
//! - [`upstream`]: 反代目标定位(`proxy_upstream` 逐键解析目标;`ProxyUpstream::resolve`
...
pub use config::{CenterClientConfig, ServiceEntry};
