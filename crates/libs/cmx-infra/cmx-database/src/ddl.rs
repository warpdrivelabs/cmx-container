...
use cmx_metadata::ddl::DdlDialect;

use crate::error::Result;
use crate::transaction::execute_sql_by_ids;
...
pub async fn execute_ddl_by_ids(
    ...
    for stmt in statements {
        execute_sql_by_ids(db_id, txn_id, stmt).await?;
    }
    Ok(())
}
...
pub async fn execute_ddl_statement_by_ids(
    ...
    execute_sql_by_ids(db_id, txn_id, statement).await
}
...
impl PgTableDefineExecutor {
    ...
    /// 执行多条 DDL 语句（内部辅助）
    fn execute_statements(&self, statements: Vec<String>) -> Result<()> {
        let db_id = self.db_id.clone();
        let txn_id = self.txn_id.clone();
        tokio::runtime::Handle::current().block_on(async move {
            for stmt in &statements {
                execute_sql_by_ids(&db_id, txn_id.as_deref(), stmt).await?;
            }
            Ok(())
        })
    }
