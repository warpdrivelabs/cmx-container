    ...
    MC: DbBmc,
{
        ...
        E: HasSeaFields,
    {
        ...
        let rows_affected = mm
            .execute_sql_with_seavalues(db_id, txn_id, &sql, sql_values)
            .await
            .map_err(|ref e| {
                if let Error::Postgres(pg_err) = e
    ...
    pub async fn get(
        ...
        let mut dataset = mm
            .query_sql_with_seavalues(db_id, txn_id, &sql, sql_values, MC::TABLE)
            .await
            .map_err(|e| {
                warn!(
                    "{:<12} - 查询失败: {}, table: {}, id: {:?}",
    ...
    F: Into<FilterGroups> + Clone + IntoFilterNodes,
{
    ...
    pub async fn count(
        ...
        let dataset = mm
            .query_sql_with_seavalues(db_id, txn_id, &sql, sql_values, "count")
            .await
            .map_err(|e| {
