...
async fn setup_client() -> RedisClient {
    let config = RedisConfig::new(get_redis_url()).with_key_prefix("cmx-buffer-test:");
...
async fn test_cache_set_ex() {
    ...
    // 设置 2 秒过期
    ops.set_ex(key, "expires soon", Duration::from_secs(2))
        .await
        .unwrap();
...
async fn test_ttl_persist() {
    ...
    // 设置带过期
    ops.set_ex(key, "permanent", Duration::from_secs(10))
        .await
        .unwrap();
