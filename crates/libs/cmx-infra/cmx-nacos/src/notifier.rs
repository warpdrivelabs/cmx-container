...
impl ConfigChangeNotifier {
    ...
    /// 通知所有处理器配置已变更
    ///
    /// 按注册顺序依次调用处理器，某个处理器失败不影响其他处理器执行。
    ///
    /// # 参数
    /// * `content` - 变更后的配置内容（TOML 格式字符串）
...
/// 全局配置变更通知器
pub struct GlobalConfigChangeNotifier;

static NOTIFIER: OnceLock<RwLock<Option<ConfigChangeNotifier>>> = OnceLock::new();

fn get_notifier() -> &'static RwLock<Option<ConfigChangeNotifier>> {
    NOTIFIER.get_or_init(|| RwLock::new(None))
}

impl GlobalConfigChangeNotifier {
    /// 初始化全局配置变更通知器
    pub fn initialize() {
        ...
        *notifier = Some(ConfigChangeNotifier::new());
        info!("全局配置变更通知器初始化完成");
    }
