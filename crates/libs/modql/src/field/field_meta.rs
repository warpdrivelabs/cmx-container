...
mod with_sea_query {
    use super::*;
    use crate::SIden;
    use sea_query::{Alias, ColumnRef, IntoColumnRef, IntoIden, SelectStatement};

    impl FieldMeta {
        pub fn sea_column_ref(&self) -> ColumnRef {
            match self.rel {
                Some(rel) => (SIden(rel), SIden(self.name())).into_column_ref(),
                None => SIden(self.name()).into_column_ref(),
            }
        }
