    ...
    impl OpValueToOpValType for OpValString {
            ...
            Self: Sized,
        {
            ...
            let ov = match (op, value) {
                ...
                ("$notStartsWithCi", Value::String(string_v)) => {
                    OpValString::NotStartsWithCi(string_v)
                }
    ...
    impl OpValString {
        pub fn into_sea_cond_expr(
            ...
            let binaries_fn = |op: BinOper, v: Vec<String>| {
                let vxpr_list: Vec<SimpleExpr> = v
                    .into_iter()
            ...
            let cond_any_of_fn =
                |op: BinOper, values: Vec<String>, val_prefix: &str, val_suffix: &str| {
            ...
            let cond = match self {
                ...
                OpValString::NotContainsAny(values) => {
                    cond_any_of_fn(BinOper::NotLike, values, "%", "%")
                }

                OpValString::StartsWith(s) => binary_fn(BinOper::Like, format!("{s}%")),
                OpValString::StartsWithAny(values) => {
                    cond_any_of_fn(BinOper::Like, values, "", "%")
                }

                OpValString::NotStartsWith(s) => binary_fn(BinOper::NotLike, format!("{s}%")),
                OpValString::NotStartsWithAny(values) => {
                    cond_any_of_fn(BinOper::NotLike, values, "", "%")
                }
                ...
                OpValString::NotEndsWithAny(values) => {
                    cond_any_of_fn(BinOper::NotLike, values, "%", "")
                }
