    ...
    impl FilterNode {
        pub fn into_sea_cond_expr_list(self) -> SeaResult<Vec<Condition>> {
            let col: ColumnRef = match self.rel {
                Some(rel) => ColumnRef::Column(ColumnName(
                    Some(rel.into()),
                    StringIden(self.name).into_iden(),
                )),
                    ...
                    OpVal::Value(ov) => {
                        ...
                        match for_sea_cond {
                            ForSeaCondition::ToSeaValue(to_sea_value) => {
                                OpValValue::into_sea_cond_expr_with_json_to_sea(
                                    ov,
                                    &col,
                                    node_options,
                                    to_sea_value,
                            ...
                            ForSeaCondition::ToSeaCondition(to_sea_condition) => {
                                to_sea_condition.call(&col, ov)?
                            }
                        }
                    }
                };
