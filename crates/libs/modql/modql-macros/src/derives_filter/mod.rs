    ...
    for field in fields.named.iter() {
        ...
        if type_name.starts_with("Option ") && type_name.contains("OpVal") {
            if let Some(ident) = field.ident.as_ref() {
                ...
                let quote_filter_node_options_cast_as =
                    if let Some(cast_as) = modql_field_attr.cast_as {
                        quote! { Some(#cast_as.to_string()) }
                    } else {
                        quote! { None }
                    };

                let quote_filter_node_options_cast_column_as =
                    if let Some(cast_column_as) = modql_field_attr.cast_column_as {
                ...
                if cfg!(feature = "with-sea-query") {
                    ...
                    let block = if let Some(to_sea_condition_fn) =
                        modql_field_attr.to_sea_condition_fn
                    {
                        let to_sea_condition_fn =
                            syn::Ident::new(&to_sea_condition_fn, proc_macro2::Span::call_site());
                        quote! {
                            // None
                        ...
                        let to_sea_value_fn =
                            syn::Ident::new(&to_sea_value_fn, proc_macro2::Span::call_site());
                        quote! {
                            // None
