...
pub async fn control_deploy(
        ...
        match name.as_str() {
            ...
            "target_db_id" => {
                let val = field
                    .text()
                    .await
                    .map_err(|e| cmx_api_core::Error::BadRequest(format!("读取 target_db_id 失败: {}", e)))?;
                if !val.is_empty() {
            ...
            "build_type" => {
                let val = field
                    .text()
                    .await
                    .map_err(|e| cmx_api_core::Error::BadRequest(format!("读取 build_type 失败: {}", e)))?;
                build_type = Some(val);
            }
            "app_id" => {
                let val = field
                    .text()
                    .await
                    .map_err(|e| cmx_api_core::Error::BadRequest(format!("读取 app_id 失败: {}", e)))?;
                if !val.is_empty() {
                    app_id = Some(val);
                }
            }
    ...
    let file_bytes = file_bytes.ok_or_else(|| {
        cmx_api_core::Error::BadRequest("未上传文件，请上传插件 zip 包".to_string())
    })?;
    ...
    let file_info = storage_service.upload(upload_request).await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("上传插件文件至对象存储失败: {}", e)))?;
    ...
    let result = control_service.deploy(control_req).await.map_err(|e| {
        cmx_api_core::Error::InternalError(format!("管控部署失败: {}", e))
    })?;
...
pub async fn control_install(
    ...
    let result = control_service.install(control_req).await.map_err(|e| {
        cmx_api_core::Error::InternalError(format!("管控安装失败: {}", e))
    })?;
...
pub async fn control_upgrade(
    ...
    let result = control_service.upgrade(control_req).await.map_err(|e| {
        cmx_api_core::Error::InternalError(format!("管控升级失败: {}", e))
    })?;
...
pub async fn control_downgrade(
    ...
    let result = control_service.downgrade(control_req).await.map_err(|e| {
        cmx_api_core::Error::InternalError(format!("管控降级失败: {}", e))
    })?;
...
pub async fn control_uninstall(
    ...
    control_service.uninstall(control_req).await.map_err(|e| {
        cmx_api_core::Error::InternalError(format!("管控卸载失败: {}", e))
    })?;
