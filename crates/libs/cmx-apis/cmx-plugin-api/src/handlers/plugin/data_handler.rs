...
fn parse_category(s: &str) -> Result<ResourceDataCategory> {
    ResourceDataCategory::parse_from_str(s).ok_or_else(|| {
        cmx_api_core::Error::BadRequest(format!(
...
pub async fn import_resource_data(
    ...
    let importer = cmx_state
        ...
        .ok_or_else(|| cmx_api_core::Error::InternalError("ResourceDataImporter 未初始化".to_string()))?;
    ...
    let zip_data =
        file_data.ok_or_else(|| cmx_api_core::Error::BadRequest("缺少 file 字段".to_string()))?;

    if domain_code.is_empty() || application_code.is_empty() || module_code.is_empty() {
        return Err(cmx_api_core::Error::BadRequest(
    ...
    let parsed_category = match category.as_deref() {
        Some(s) => parse_category(s)
            .map_err(|e| cmx_api_core::Error::BadRequest(format!("无效的 category: {e}")))?,
    ...
    let result = importer
        .import_data(request)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(e.to_string()))?;
