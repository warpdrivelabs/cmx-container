...
use cmx_storage::handler::AppState as StorageAppState;
...
/// 从 CmxAppState 提取 cmx-storage 的 AppState（axum 状态自动注入）。
///
/// 注：此 impl 必须定义在 CmxAppState 的本地 crate（cmx-api-core）以满足孤儿规则
/// （CmxAppState 与 StorageAppState 均非 cmx-api 本地类型时，cmx-api 无法 impl）。
/// 从 cmx-api/src/handlers/storage/mod.rs 迁入。
impl FromRef<CmxAppState> for StorageAppState {
