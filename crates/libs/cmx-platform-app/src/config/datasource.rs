...
pub async fn init_datasources() -> crate::Result<()> {
    ...
    // 读取本实例应用标识（micro 模式下用于数据源过滤；mono 模式下不生效）。
    let app_identity = crate::config::load_app_identity();

    // micro 模式校验：[app] 三元组必须配置具体值，不能为 default。
            ...
            || app_identity.module_code == "default"
        {
            return Err(Error::DatasourceInit(format!(
                "micro 模式下 [app] 三元组必须配置具体值（当前: {}/{}/{}），\
                 禁止使用 default 占位",
                app_identity.domain_code,
                app_identity.application_code,
                app_identity.module_code
            )));
        }
    }

    for c in &mut configs {
        // mono 模式不强制覆盖 D-A-M（保留配置文件原值，为未来分体化预留元数据）；
        // micro 模式维持原兜底逻辑（未显式声明则继承 [app] 三元组）。
        if deploy_mode == DeployMode::Micro {
    ...
    if let Err(e) = persist_datasource_configs(
        db_manager,
        &default_db_id,
        &app_identity,
        deploy_mode,
    ...
    let active_datasources = load_active_datasources(
        db_manager,
        &default_db_id,
        &app_identity,
        deploy_mode,
...
///
/// 执行以下操作：
/// 1. **upsert**：对配置文件中的每个数据源查重，存在则更新、不存在则插入。
///    - mono 模式：按 `db_id + source='config'` 查重（全局唯一）
///    - micro 模式：按 `db_id + 域应用模块` 查重（同 db_id 可在不同域下重复）
/// 2. **清理**：查询数据库中 `source="config"` 的记录，
///    将配置文件中已删除的（db_id 不在配置文件中）物理删除，避免残留无效数据源。
///
/// # Arguments
///
/// * `mm` - 数据库管理器
/// * `db_id` - 默认数据库ID
/// * `app_identity` - 当前实例应用标识（micro 模式用）
/// * `deploy_mode` - 部署模式
...
async fn persist_datasource_configs(
    ...
    info!("开始持久化数据源配置（模式: {:?}）...", deploy_mode);

    for config in configs {
        // 查重键按部署模式分支
        let filter = if deploy_mode == DeployMode::Mono {
            // mono 模式：仅按 db_id + source='config' 查重（全局唯一）
            SysDatasourceFilter {
            ...
            // micro 模式：按 db_id + 域应用模块联合查重（db_id 在不同域下可重复）
            SysDatasourceFilter {
                db_id: Some(OpValsString(vec![OpValString::Eq(config.db_id.clone())])),
    ...
    // 清理：查询 source="config" 的记录，配置文件中已删除的（db_id 不在配置文件中）物理删除。
    // mono 模式：全局清理（不按 D-A-M）
    // micro 模式：仅清理本域应用模块的记录
    let config_db_ids: Vec<String> = configs.iter().map(|c| c.db_id.clone()).collect();

    let cleanup_filter = if deploy_mode == DeployMode::Mono {
        SysDatasourceFilter {
            source: Some(OpValsString(vec![OpValString::Eq("config".to_string())])),
            ..Default::default()
        }
...
///
/// 查询条件：
/// - mono 模式：`status=1 AND archived=0`（加载全部，不按 D-A-M 过滤）
/// - micro 模式：`status=1 AND archived=0 AND domain_code=? AND application_code=? AND module_code=?`
///
/// # Arguments
///
/// * `mm` - 数据库管理器
...
async fn load_active_datasources(
    ...
    // 过滤条件按部署模式分支
    let filter = if deploy_mode == DeployMode::Mono {
        // mono 模式：仅按 status/archived 过滤，加载全部数据源
        SysDatasourceFilter {
        ...
        // micro 模式：按 D-A-M + status/archived 精确过滤
        SysDatasourceFilter {
            domain_code: Some(OpValsString(vec![OpValString::Eq(
                app_identity.domain_code.clone(),
            )])),
...
async fn register_pg_datasources(configs: Vec<DbConfig>) {
    ...
    // sqlx 形态 DbConfig → pg 形态（两 crate 字段一致，逐字段映射），再委托 base 库统一注册。
    let pg_configs: Vec<PgDbConfig> = configs
        .into_iter()
        .filter(|c| matches!(c.db_type, cmx_database::DbType::Postgres))
        .map(|c| PgDbConfig {
