...
impl DefinitionStore for MemStore {
    ...
    async fn upsert_draft(&self, rec: &DefinitionRecord) -> DefResult<()> {
        self.defs.lock().unwrap().insert(rec.key.clone(), rec.clone());
        Ok(())
    }
    ...
    async fn load_published(&self) -> DefResult<Vec<PublishedEntry>> {
        ...
        for r in defs.values() {
            if r.state == DefinitionState::Published {
                if let Some(av) = r.active_version {
                    if let Some(v) = vers.iter().find(|v| v.def_key == r.key && v.version == av) {
...
async fn save_draft_rejects_invalid_bpmn() {
    ...
    // 非良构 XML → 试编译挡回。
    let r = svc.save_draft("坏流程", None, None, None, None, "<not-bpmn/>", None).await;
...
async fn save_draft_stores_and_derives_key_from_process_id() {
    let svc = DefinitionService::new(MemStore::default());
    let rec = svc
        .save_draft("请假申请", Some("fi".into()), Some("cmxfico".into()), Some("hr".into()), None, VALID_BPMN, Some("alice".into()))
        .await
...
async fn publish_bumps_version_and_marks_published() {
    let svc = DefinitionService::new(MemStore::default());
    svc.save_draft("请假申请", None, None, None, None, VALID_BPMN, None).await.unwrap();

    // 首次发布 → v1。
    let v1 = svc.publish("leave_request", None, Some("bob".into())).await.unwrap();
    assert_eq!(v1, 1);
    ...
    svc.save_draft("请假申请v2", None, None, None, None, VALID_BPMN, None).await.unwrap();
    let v2 = svc.publish("leave_request", None, None).await.unwrap();
    assert_eq!(v2, 2);
    assert!(svc.get_version("leave_request", 1).await.unwrap().is_some(), "旧版本保留");
...
async fn version_list_activate_and_delete() {
    let svc = DefinitionService::new(MemStore::default());
    svc.save_draft("请假申请", None, None, None, None, VALID_BPMN, None).await.unwrap();
    svc.publish("leave_request", Some("初版".into()), None).await.unwrap(); // v1
    svc.save_draft("请假申请", None, None, None, None, VALID_BPMN, None).await.unwrap();
    svc.publish("leave_request", Some("改办理人".into()), None).await.unwrap(); // v2
    ...
    assert!(svc.delete_version("leave_request", 2).await.is_err(), "当前版本不能删");
