...
fn normalize_decision(raw: &Value) -> Result<Value, String> {
    ...
    match kind {
        ...
        "approval" => {
            ...
            if !["run_command", "apply_json_patch", "apply_text_replace"].contains(&action) {
...
pub async fn build_summary(events: &[Value], context: &Value, messages: &[Value]) -> String {
    if std::env::var("CMX_AGENT_PLANNER").ok().as_deref() == Some("llm") && crate::ai::is_configured() {
        match llm_summary(events, context, messages).await {
            Ok(s) => return s,
...
async fn llm_summary(events: &[Value], context: &Value, messages: &[Value]) -> PortalResult<String> {
    // 压缩工具事件（截断大字段）
    ...
    let content = crate::ai::raw_chat_completion(req, false, 0.2).await?;
