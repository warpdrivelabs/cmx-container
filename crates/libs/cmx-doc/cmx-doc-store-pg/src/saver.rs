    ...
    async fn apply_merge(
        ...
        for (idx, layer_id) in meta.layer_order.iter().enumerate() {
            ...
            if let Some(rows) = layer_changes.get("inserted").and_then(|v| v.as_array())
                && !rows.is_empty()
            {
                // 对账口径一致：无有效列的行（upsert_rows 内部会跳过）不纳入期望数。
                let effective: Vec<Value> = rows
                    .iter()
                    .filter(|r| row_has_effective_columns(r, layer, true))
                    .cloned()
                    .collect();
                if !effective.is_empty() {
                    write_expected += effective.len() as u64;
                    write_affected +=
                        Self::upsert_rows(mm, db_id, txn_id, layer, &effective, audit).await?;
                }
            }
            if let Some(rows) = layer_changes.get("updated").and_then(|v| v.as_array())
                && !rows.is_empty()
            {
                // 对账口径一致：fields ∩ schema 为空的行（update_rows 内部"无业务变更列 →
                // 跳过整行"）不纳入期望数——否则恒报「期望 N 实际 0」的假并发冲突。
                // 典型来源：前端脏标记了行但未取到任何字段值（fields 导出为 {}）。
                let effective: Vec<Value> = rows
                    .iter()
                    .filter(|r| row_has_effective_columns(r, layer, false))
                    .cloned()
                    .collect();
                if !effective.is_empty() {
                    write_expected += effective.len() as u64;
                    ...
                    let oplock = idx == 0;
                    write_affected +=
                        Self::update_rows(mm, db_id, txn_id, layer, &effective, audit, oplock)
                            .await?;
                }
            }
        }
...
/// 行的"有效写入列"判定（与 `DocSaver::row_cols_vals` / `DocSaver::update_rows` 的跳过口径一致）：
/// - inserted（include_top_keys=true）：顶层 id/upper_id/line_no（schema 内）或 fields∩schema 任一存在；
/// - updated（include_top_keys=false）：仅 fields∩schema（顶层 id 是 WHERE 键，不计入 SET）。
///
/// 用于 apply_merge 的写入对账——无有效列的行会在批量写入内核里被跳过，
/// 期望行数若仍按原始行数计，会恒报「期望 N 实际 0」的假并发冲突。
fn row_has_effective_columns(row: &Value, layer: &LayerView, include_top_keys: bool) -> bool {
    if include_top_keys
        && ["id", "upper_id", "line_no"].iter().any(|top| {
            layer.schema.get_index(top).is_some() && row.get(*top).is_some()
        })
    {
        return true;
    }
    row.get("fields")
        .and_then(|v| v.as_object())
        .map(|f| f.keys().any(|c| layer.schema.get_index(c).is_some()))
        .unwrap_or(false)
}
...
mod tests {
    ...
    #[test]
    fn row_has_effective_columns_matches_skip_semantics() {
        let doc = json!({
            ...
            "voucherTables": [ { "level":"L1", "tableName":"t",
                "fields":[
                    ...
                    {"name":"doc_type_id","dataType":"VARCHAR"}
                ] } ]
        });
        let view = DocMetaView::parse(&doc, &json!(null)).unwrap();
        let layer = view.layer("t").expect("layer 应存在");

        // updated（仅 fields 计入 SET）：空 fields / 全非 schema 键 → 无效（update_rows 跳过，不纳入对账）
        assert!(!row_has_effective_columns(&json!({"id":1,"fields":{}}), &layer, false));
        assert!(!row_has_effective_columns(&json!({"id":1,"fields":{"not_a_col":1}}), &layer, false));
        // updated：含 schema 键 → 有效
        assert!(row_has_effective_columns(&json!({"id":1,"fields":{"doc_type_id":"SA"}}), &layer, false));
        // inserted（row_cols_vals 还会写顶层 id/upper_id/line_no）：仅顶层 id 也算有效
        assert!(row_has_effective_columns(&json!({"id":1,"fields":{}}), &layer, true));
        assert!(row_has_effective_columns(&json!({"id":1,"fields":{"doc_type_id":"SA"}}), &layer, true));
    }
