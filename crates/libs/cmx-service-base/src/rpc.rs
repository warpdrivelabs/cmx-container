...
pub async fn init_rpc(
    ...
    match tokio::time::timeout(std::time::Duration::from_secs(3), server_ready_rx).await {
        ...
        Ok(Err(_)) => {
            server_handle.abort();
            return Err(BaseError::Setup("gRPC Server 启动失败".to_string()));
        }
        Err(_) => {
            server_handle.abort();
            return Err(BaseError::Setup("gRPC Server 启动超时".to_string()));
        }
    }
...
///
/// 缺省返回空配置（`outgoing_api_key` 为空字符串）。公开供 portal 侧
/// `load_outgoing_credential`（绑 cmx-plugin `Credential`，留 portal）复用。
