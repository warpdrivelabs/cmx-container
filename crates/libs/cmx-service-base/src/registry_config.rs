//! 全局配置 / 注册中心 / 配置中心初始化（feature `registry-config`）。
//!
//! 从 web-server `config/infra_init.rs` **原样提取的通用部分**：加载并合并多源配置
//! （本地 TOML + 远程配置中心 + 环境变量）→ 初始化 `ConfigManager`；创建注册中心/配置中心
//! 单例；注册当前服务实例；启动服务列表定时同步 + 配置热更新监听。这是任何微服务起服的
//! **第一个** init 入口（Nacos/配置中心是通用微服务能力，非 portal 专属）。
//!
//! ★ 与 rpc 的唯一交叉：`inject_rpc_metadata` 需读 `[rpc]` 段拿 `grpc_port` 注入注册元数据。
//! rpc 也在本 crate（feature `rpc`），故用 `#[cfg(feature = "rpc")]` 内部条件编译解决——
//! 只在 rpc feature 也开时注入 grpc_port，registry-config 单开时不注入。本模块**不依赖任何
//! 业务 crate**（cmx-api/cmx-iam/cmx-biz 一个不碰）。
...
///
/// 该函数是微服务 `main.rs` 启动后**第一个**被调用的初始化入口，负责：
/// - 加载并合并多源配置（本地 TOML + 远程配置中心 + 环境变量）
/// - 创建并存储注册中心、配置中心实例到全局单例
/// - 注册服务实例到注册中心
/// - 启动配置变更监听（Nacos 热更新）
///
/// # Returns
///
/// * `Ok(())` - 全部初始化成功。
/// * `Err(BaseError::Config)` - 配置加载、合并或单例设置失败。
pub async fn init_infra() -> Result<()> {
    ...
    let (registry, cache) = create_registry_with_cache(&registry_config)
        .await
        .map_err(|e| BaseError::Config(format!("创建注册中心失败: {}", e)))?;
    GlobalServiceInstanceCache::set(cache)
        .map_err(|e| BaseError::Config(format!("设置全局服务实例缓存失败: {}", e)))?;
    ...
    let config_center = create_config_center(&cc_config, change_handler)
        .await
        .map_err(|e| BaseError::Config(format!("创建配置中心失败: {}", e)))?;
...
///
/// 从全局配置加载 `[registry.metadata]` 段的用户自定义 metadata，合并到
/// `registry_config.metadata`。当 feature `rpc` 也开启时，额外从 `[rpc]` 段读取 `grpc_port`
/// 注入（优先级高于配置文件的值），确保注册和注销时 metadata 一致。
///
/// ★ registry-config 单开（不开 rpc）时，`grpc_port` 注入分支被 `#[cfg]` 编译掉——
/// 保证本模块**不硬依赖** rpc 模块的存在。
fn inject_rpc_metadata(registry_config: &mut RegistryConfig) {
    ...
    // RPC 自动注入的 metadata 优先级高于配置文件中的值（仅当 rpc feature 开启）。
    #[cfg(feature = "rpc")]
    if let Some(rpc) = crate::rpc::load_rpc_config()
        && rpc.enabled
    {
        registry_config
            .metadata
