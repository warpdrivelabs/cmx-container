...
pub async fn plugin_uninstall(
    ...
    let result = manager
        ...
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("插件卸载失败: {}", e)))?;
...
pub async fn plugin_upgrade(
    ...
    let result = manager
        .upgrade(upgrade_req)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("插件升级失败: {}", e)))?;
...
pub async fn plugin_downgrade(
    ...
    let result = manager
        ...
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("插件降级失败: {}", e)))?;
...
pub async fn plugin_deploy(
        ...
        match name.as_str() {
            ...
            "target_db_id" => {
                let val = field.text().await.map_err(|e| {
                    cmx_api_core::Error::BadRequest(format!("读取 target_db_id 失败: {}", e))
                })?;
                if !val.is_empty() {
            ...
            "force_reinstall" => {
                let val = field.text().await.map_err(|e| {
                    cmx_api_core::Error::BadRequest(format!("读取 force_reinstall 失败: {}", e))
                })?;
                force_reinstall = val == "true" || val == "1";
            }
            "build_type" => {
                let val = field.text().await.map_err(|e| {
                    cmx_api_core::Error::BadRequest(format!("读取 build_type 失败: {}", e))
                })?;
                build_type = Some(val);
            }
            "publish_to_marketplace" => {
                let text = field
                    .text()
                    .await
                    .map_err(|e| cmx_api_core::Error::internal_error(format!("读取字段失败: {}", e)))?;
    ...
    let file_bytes = file_bytes
        .ok_or_else(|| cmx_api_core::Error::BadRequest("未上传文件，请上传插件 zip 包".to_string()))?;

    // 确保上传目录存在
    tokio::fs::create_dir_all(&uploads_dir)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("创建上传目录失败: {}", e)))?;
    ...
    tokio::fs::write(&file_path, &file_bytes)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("保存文件失败: {}", e)))?;

    let abs_path = tokio::fs::canonicalize(&file_path)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("获取文件绝对路径失败: {}", e)))?;
    ...
    if publish_to_marketplace.unwrap_or(true) {
        ...
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("解析插件定义失败: {}", e)))?
        .map_err(|e| cmx_api_core::Error::InternalError(format!("解析插件定义失败: {}", e)))?;
        ...
        let result =
            ...
            .await
            .map_err(|e| cmx_api_core::Error::InternalError(format!("发布到插件市场失败: {}", e)))?;
    ...
    let result = manager
        .deploy(deploy_req)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("插件部署失败: {}", e)))?;
...
pub async fn plugin_list(
    ...
    let plugins = manager
        ...
        .list_plugins(&filter)
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("获取插件列表失败: {}", e)))?;
...
pub async fn plugin_get(
    ...
    let plugin = manager
        ...
        .await
        .map_err(|e| cmx_api_core::Error::InternalError(format!("获取插件详情失败: {}", e)))?;

    match plugin {
        Some(info) => Ok(Json(ApiResp::ok(convert_plugin_info(info)))),
        // None => Err(cmx_api_core::Error::NotFound(format!("插件 {} 不存在", params.plugin_id))),
...
pub async fn plugin_exists(
    ...
    let exists = manager
        ...
        .await
        .map_err(|e| cmx_api_core::Error::internal_error(format!("查询插件存在性失败: {}", e)))?;
