...
impl ModuleRoutes for PortalModule {
    fn routes(self) -> Router<CmxAppState> {
        Router::new()
            ...
            // 工作区节点
            .route("/workspace-nodes", get(handler::list_workspace_nodes).post(handler::save_workspace_node))
            .route(
                "/workspace-nodes/{id}",
                get(handler::get_workspace_node).delete(handler::delete_workspace_node),
            )
            // 表单页
            .route("/form-pages", get(handler::list_form_pages).post(handler::save_form_page))
            .route("/form-pages/{id}", get(handler::get_form_page))
            // 原生页面
            .route("/native-pages", get(handler::list_native_pages).post(handler::save_native_page))
            .route("/native-pages/batch", post(handler::batch_native_pages))
            .route("/native-pages/{id}", get(handler::get_native_page))
            // HTML 页面（v2 分片 + v1 兼容）
            .route("/html-pages", get(handler::list_html_pages).post(handler::save_html_page))
            ...
            .route("/fact/{domain}/{app}/{module}/{file}", get(handler::get_fact_path))
            ...
            .route("/dam-registry/domains", get(handler::dam_list_domains).post(handler::dam_upsert_domain))
            .route("/dam-registry/domains/{domain}", axum::routing::delete(handler::dam_delete_domain))
            .route("/dam-registry/applications", get(handler::dam_list_applications).post(handler::dam_upsert_application))
            .route("/dam-registry/applications/{domain}/{application}", axum::routing::delete(handler::dam_delete_application))
            .route("/dam-registry/modules", get(handler::dam_list_modules).post(handler::dam_upsert_module))
            .route("/dam-registry/modules/{domain}/{application}/{module}", axum::routing::delete(handler::dam_delete_module))
            // 注册表只读派生
            ...
            .route("/modules/{domain}/{application}/{module}", get(handler::get_module_manifest))
            .route("/modules/{domain}/{application}/{module}/resources/{type}", get(handler::get_module_resource))
            .route("/module-resources", get(handler::module_resources))
            // 定义中心（DCT/DOC/BASE）
            .route("/definitions/list", get(handler::definitions_list))
            .route(
                "/definitions/config",
                get(handler::definitions_get).post(handler::definitions_save).delete(handler::definitions_delete),
            )
            .route("/definitions/batch", post(handler::definitions_batch))
            .route("/definitions/default", post(handler::definitions_set_default))
            // 字典检索服务
            ...
            .route("/dict/{dictId}/entries/{id}", axum::routing::delete(handler::dict_delete_entry))
            ...
            .route(
                ...
                get(handler::cp_get_config).post(handler::cp_save_config).delete(handler::cp_delete_config),
            )
