...
pub async fn fc_get_config(
    ...
    Ok(Json(ApiResp::ok(cmx_portal::flexible_combination::store::get_flexible_combination(&q.to_ref()).await?)))
}
...
pub async fn fc_save_config(
    ...
    let saved = cmx_portal::flexible_combination::store::save_flexible_combination(&q.to_ref(), &body).await?;
    Ok(Json(ApiResp::ok(serde_json::json!({ "ok": true, "saved": saved }))))
}
...
pub async fn fc_delete_config(
    ...
    Ok(Json(ApiResp::ok(cmx_portal::flexible_combination::store::delete_flexible_combination(&q.to_ref()).await?)))
}
...
pub async fn fc_resolve(
    ...
    Ok(Json(ApiResp::ok(cmx_portal::flexible_combination::api::resolve(&q.to_ref(), &q.anchor_map()).await?)))
}
...
pub async fn fc_rule(
    ...
    Ok(Json(ApiResp::ok(cmx_portal::flexible_combination::api::rule(&q.to_ref(), &q.anchor_map()).await?)))
}
...
pub async fn fc_preview(
    ...
    Ok(Json(ApiResp::ok(cmx_portal::flexible_combination::api::preview(&body, &q.to_ref()).await?)))
}
