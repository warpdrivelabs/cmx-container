...
pub async fn create_role_group(
    ...
    debug!(
        "{:<12} - handler::create_role_group - name: {}",
        "HANDLER", data.name
    );
...
pub async fn update_role_group(
    ...
    debug!(
        "{:<12} - handler::update_role_group - id: {}",
        "HANDLER", role_group_id
    );
...
pub async fn delete_role_group(
    ...
    debug!(
        "{:<12} - handler::delete_role_group - count: {}",
        "HANDLER",
        role_group_ids.len()
    );
...
pub async fn page_role_groups(
    ...
    let (role_groups, total) = iam
        .role_group_service
        .page_role_groups(filters, list_options)
        .await
        .map_err(|e| Error::business_error(e.to_string()))?;
...
pub async fn list_role_groups(
    ...
    let role_groups = iam
        .role_group_service
        .list_role_groups(filters, Some(list_options))
        .await
        .map_err(|e| Error::business_error(e.to_string()))?;
