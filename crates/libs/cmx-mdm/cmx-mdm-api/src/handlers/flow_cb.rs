...
use cmx_api_core::db_id::resolve_db_id;
...
pub async fn mdm_cr_withdraw(
    ...
    let mm = get_default_pg_db_manager();
    let db_id = cmx_api_core::db_id::resolve_db_id_from_headers(&headers).await;
    ...
    let current = current_user_id(&svr_ctx);
...
pub(crate) fn current_user_id(svr_ctx: &CmxSvrContext) -> Option<String> {
    svr_ctx
        .0
        .auth_context
        .as_ref()
