//! 维度 dict 元数据补全（复刻处理器侧 `enrichContextProfileDictMeta` / `enrichDimensionDictMeta`）。
//!
...
/// 对整份 profile 的 dimensions 做 dict 元数据补全（返回新对象，不改原值）。
pub async fn enrich_context_profile_dict_meta(cfg: &Value) -> PortalResult<Value> {
    if !cfg.is_object() {
        return Ok(cfg.clone());
    }
