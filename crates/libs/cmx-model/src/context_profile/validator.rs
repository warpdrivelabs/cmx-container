...
const COLUMN_TYPES: &[&str] = &["text", "number", "date", "boolean", "select", "ref", "combo", "dict-select"];
...
const EDIT_MODES: &[&str] = &[
    "cmx-text-input", "cmx-textarea-input", "cmx-richtext-input", "cmx-number-input", "cmx-date-input",
    "cmx-datetime-input", "select", "ref", "combo", "ignite-combo", "cmx-dict-selct", "checkbox", "image",
    "video", "readonly", "none", "computed", "tree-ref",
];
...
impl Diag {
    fn new() -> Diag {
        Diag { errors: Vec::new(), warnings: Vec::new() }
    }
    fn error(&mut self, path: &str, code: &str, message: impl Into<String>) {
        self.errors.push(json!({ "path": path, "code": code, "message": message.into() }));
    }
    fn warn(&mut self, path: &str, code: &str, message: impl Into<String>) {
        self.warnings.push(json!({ "path": path, "code": code, "message": message.into() }));
    }
...
fn field_id(field: &Value) -> String {
    field.get("id").and_then(|v| v.as_str()).map(|s| s.to_string())
        .or_else(|| field.get("id").map(|v| match v { Value::Number(n) => n.to_string(), _ => String::new() }))
        .unwrap_or_default()
}
...
pub fn validate_context_profile(profile: &Value) -> Value {
    ...
    for (dim_code, dim) in dimensions.as_object().unwrap() {
        ...
        if dim.get("name").is_none() && dim.get("caption").is_none() {
            d.warn(&base, "DIMENSION_NAME_MISSING", format!("维度 {dim_code} 未设置 name/caption"));
        }
        if let Some(attrs) = dim.get("attributes")
            && !attrs.is_null() && !attrs.is_array() && !attrs.is_object() {
                d.error(&format!("{base}.attributes"), "DIMENSION_ATTRIBUTES_INVALID", "attributes 必须是数组或对象");
            }
        if let Some(values) = dim.get("values")
            && !values.is_null() && !values.is_array() {
                d.error(&format!("{base}.values"), "DIMENSION_VALUES_INVALID", "values 必须是数组");
            }
    }
    ...
    for (i, dd) in profile_anchor_dims.iter().enumerate() {
        if !dim_codes.contains(dd) {
            d.error(&format!("anchorDimensions.{i}"), "ANCHOR_DIMENSION_UNKNOWN", format!("锚点维度 {dd} 未在 dimensions 中定义"));
        }
    }
    ...
    for (rule_index, rule) in rules_arr.iter().enumerate() {
        ...
        match rule.get("id").and_then(|v| v.as_str()) {
            ...
            Some(id) => {
                if rule_ids.contains(id) {
                    d.error(&format!("{r_path}.id"), "RULE_ID_DUPLICATE", format!("规则 id 重复：{id}"));
                } else {
        ...
        for (i, dd) in anchor_dims.iter().enumerate() {
            if !dim_codes.contains(dd) {
                d.error(&format!("{r_path}.anchor.dimensions.{i}"), "ANCHOR_DIMENSION_UNKNOWN", format!("锚点维度 {dd} 未在 dimensions 中定义"));
            }
        }
        let anchor_cols: Vec<String> = rule
            ...
            .and_then(|v| v.as_array())
            .map(|a| {
                a.iter().enumerate().filter_map(|(i, c)| {
                    match c.as_str() {
                        Some(s) if !s.trim().is_empty() => Some(s.to_string()),
                        _ => {
                            d.error(&format!("{r_path}.anchor.columns.{i}"), "ANCHOR_COLUMN_INVALID", "锚点列必须是非空字符串");
                            None
                        }
                    }
                }).collect()
        ...
        if anchor_dims.is_empty() && anchor_cols.is_empty() {
            d.warn(&format!("{r_path}.anchor"), "ANCHOR_EMPTY", "规则未声明锚点维度/列，解析时只能作为兜底规则");
        }
        // valid match keys
        let mut valid_match_keys: HashSet<String> = anchor_cols.iter().cloned().chain(anchor_dims.iter().cloned()).collect();
        ...
        check_match_object(rule.get("anchor").and_then(|a| a.get("match")), &format!("{r_path}.anchor.match"), &valid_match_keys, &mut d);

        validate_column_model_props(rule.get("columnModel"), &format!("{r_path}.columnModel"), &mut d);

        let fields = rule.get("detail").and_then(|de| de.get("fields"));
        if !fields.map(|v| v.is_array()).unwrap_or(false) {
            d.error(&format!("{r_path}.detail.fields"), "FIELDS_REQUIRED", "规则缺少 detail.fields 数组");
        }
        let fields_arr = fields.and_then(|v| v.as_array()).cloned().unwrap_or_default();
        validate_fields(&fields_arr, &r_path, dimensions, &dim_codes, &mut d);
        validate_groups(rule.get("detail").and_then(|de| de.get("groups")), &fields_arr, &r_path, &mut d);
    }

    d.finish()
}

fn validate_fields(fields: &[Value], r_path: &str, dimensions: &Value, dim_codes: &HashSet<String>, d: &mut Diag) {
    ...
    for (fi, field) in fields.iter().enumerate() {
        ...
        if id.is_empty() {
            d.error(&format!("{f_path}.id"), "FIELD_CODE_REQUIRED", "字段缺少 id");
        } else if field_codes.contains(&id) {
            d.error(&format!("{f_path}.id"), "FIELD_CODE_DUPLICATE", format!("字段 id 重复：{id}"));
        } else {
            field_codes.insert(id.clone());
        }
        if let Some(dt) = field.get("dimType").and_then(|v| v.as_str())
            && !FIELD_KINDS.contains(&dt) {
                d.warn(&format!("{f_path}.dimType"), "FIELD_KIND_UNKNOWN", format!("未知字段 dimType：{dt}"));
            }
        if field.get("dimType").and_then(|v| v.as_str()) == Some("dimension") {
            let dim_code = field.get("refDict").and_then(|v| v.as_str()).unwrap_or(&id).to_string();
            if !dim_codes.contains(&dim_code) {
                d.error(&format!("{f_path}.refDict"), "FIELD_DIMENSION_UNKNOWN", format!("字段引用的维度 {dim_code} 未定义"));
            }
        }
        if let Some(src_dim) = field.get("source").and_then(|s| s.get("dimension")).and_then(|v| v.as_str()) {
            if !dim_codes.contains(src_dim) {
                d.error(&format!("{f_path}.source.dimension"), "SOURCE_DIMENSION_UNKNOWN", format!("source.dimension {src_dim} 未定义"));
            } else if let Some(attr) = field.get("source").and_then(|s| s.get("attribute")).and_then(|v| v.as_str())
                && !has_dimension_attribute(dimensions.get(src_dim), attr) {
                    d.warn(&format!("{f_path}.source.attribute"), "SOURCE_ATTRIBUTE_NOT_DECLARED", format!("维度 {src_dim} 未声明属性 {attr}"));
                }
        }
        if let Some(df_dim) = field.get("defaultFrom").and_then(|s| s.get("dimension")).and_then(|v| v.as_str()) {
            if !dim_codes.contains(df_dim) {
                d.error(&format!("{f_path}.defaultFrom.dimension"), "DEFAULT_DIMENSION_UNKNOWN", format!("defaultFrom.dimension {df_dim} 未定义"));
            } else if let Some(attr) = field.get("defaultFrom").and_then(|s| s.get("attribute")).and_then(|v| v.as_str())
                && !has_dimension_attribute(dimensions.get(df_dim), attr) {
                    d.warn(&format!("{f_path}.defaultFrom.attribute"), "DEFAULT_ATTRIBUTE_NOT_DECLARED", format!("维度 {df_dim} 未声明属性 {attr}"));
                }
        }
        ...
        if let Some(deps) = field.get("dependsOn").and_then(|v| v.as_array()) {
            for (di, dep) in deps.iter().enumerate() {
                ...
                let exists = field_codes.contains(dep_s) || fields.iter().any(|f| field_id(f) == dep_s);
                if !exists {
                    d.error(&format!("{f_path}.dependsOn.{di}"), "DEPENDENCY_UNKNOWN", format!("依赖字段 {dep_s} 不存在"));
                }
            }
        }
        if let Some(vals) = field.get("validations").and_then(|v| v.as_array()) {
            for (vi, rule) in vals.iter().enumerate() {
                match rule.get("expr") {
                    None | Some(Value::Null) => d.error(&format!("{f_path}.validations.{vi}.expr"), "VALIDATION_EXPR_REQUIRED", "校验规则缺少 expr"),
                    Some(expr) => {
                        if let Err(msg) = check_formula(expr) {
                            d.error(&format!("{f_path}.validations.{vi}.expr"), "VALIDATION_EXPR_INVALID", msg);
                        }
                    }
                }
            }
...
fn validate_groups(groups: Option<&Value>, fields: &[Value], r_path: &str, d: &mut Diag) {
    let Some(groups) = groups.filter(|v| !v.is_null()) else { return };
    if !groups.is_array() {
        d.error(&format!("{r_path}.detail.groups"), "GROUPS_ARRAY_REQUIRED", "groups 必须是数组");
        return;
    }
    let field_codes: HashSet<String> = fields.iter().map(field_id).filter(|s| !s.is_empty()).collect();
    let mut seen: HashSet<String> = HashSet::new();

    fn walk(node: &Value, path: &str, field_codes: &HashSet<String>, seen: &mut HashSet<String>, d: &mut Diag) {
        if !node.is_object() {
        ...
        if !members.map(|v| v.is_array()).unwrap_or(false) {
            d.error(&format!("{path}.members"), "GROUP_MEMBERS_REQUIRED", "分组缺少 members 数组");
            return;
        }
            ...
            match member {
                Value::String(s) => {
                    if !field_codes.contains(s) {
                        d.error(&m_path, "GROUP_FIELD_UNKNOWN", format!("分组引用了不存在的字段 {s}"));
                    } else if seen.contains(s) {
                        d.warn(&m_path, "GROUP_FIELD_DUPLICATE", format!("字段 {s} 被多个分组引用，将只使用首次出现"));
                    } else {
                        seen.insert(s.clone());
                    }
                }
    ...
    for (i, g) in groups.as_array().unwrap().iter().enumerate() {
        walk(g, &format!("{r_path}.detail.groups.{i}"), &field_codes, &mut seen, d);
    }
}

fn check_match_object(m: Option<&Value>, path: &str, valid_keys: &HashSet<String>, d: &mut Diag) {
    let Some(m) = m.filter(|v| !v.is_null()) else { return };
    if !m.is_object() {
    ...
    for (dim, cond) in m.as_object().unwrap() {
        if !valid_keys.is_empty() && !valid_keys.contains(dim) {
            d.error(&format!("{path}.{dim}"), "MATCH_KEY_NOT_IN_ANCHOR", format!("匹配列 {dim} 不在锚点列、锚点维度或其属性列中"));
        }
        if cond.is_object() && !cond.is_array() {
            ...
            if ops > 0 && attrs > 0 {
                d.error(&format!("{path}.{dim}"), "MATCH_OPERATOR_MIXED", "匹配条件不能同时混用操作符和属性条件");
            }
        }
    }
}
...
fn validate_column_props(column: Option<&Value>, path: &str, d: &mut Diag) {
    let Some(column) = column.filter(|v| !v.is_null()) else { return };
    if !column.is_object() {
    ...
    if let Some(t) = column.get("type").and_then(|v| v.as_str())
        && !COLUMN_TYPES.contains(&t) {
            d.warn(&format!("{path}.type"), "COLUMN_TYPE_UNKNOWN", format!("未知列类型：{t}"));
        }
    if let Some(a) = column.get("agg").and_then(|v| v.as_str())
        && !COLUMN_AGGS.contains(&a) {
            d.error(&format!("{path}.agg"), "COLUMN_AGG_INVALID", format!("agg 必须是 sum/count/avg/max/min，实际：{a}"));
        }
    if let Some(f) = column.get("frozen")
        && !f.is_null() && !f.is_boolean() {
            d.error(&format!("{path}.frozen"), "COLUMN_FROZEN_INVALID", "frozen 必须是布尔值");
        }
    if let Some(v) = column.get("visible")
        && !v.is_null() && !v.is_boolean() {
            d.error(&format!("{path}.visible"), "COLUMN_VISIBLE_INVALID", "visible 必须是布尔值");
        }
    if let Some(w) = column.get("width")
        && !w.is_null() && !w.is_string() && !w.is_number() && !w.is_object() {
            d.error(&format!("{path}.width"), "COLUMN_WIDTH_INVALID", "width 必须是字符串/数字/对象");
        }
    if let Some(disp) = column.get("display").filter(|v| !v.is_null()) {
        if !disp.is_object() {
            d.error(&format!("{path}.display"), "COLUMN_DISPLAY_INVALID", "display 必须是对象");
        } else {
            if let Some(al) = disp.get("align").and_then(|v| v.as_str())
                && !COLUMN_ALIGNS.contains(&al) {
                    d.error(&format!("{path}.display.align"), "COLUMN_DISPLAY_ALIGN_INVALID", "display.align 必须是 left/center/right");
                }
            if let Some(md) = disp.get("mode").and_then(|v| v.as_str())
                && !DISPLAY_MODES.contains(&md) {
                    d.warn(&format!("{path}.display.mode"), "COLUMN_DISPLAY_MODE_UNKNOWN", format!("未知 display.mode：{md}"));
                }
            match disp.get("cellStyle") {
                Some(cs) if !cs.is_null() && !cs.is_array() => {
                    d.error(&format!("{path}.display.cellStyle"), "COLUMN_CELLSTYLE_INVALID", "display.cellStyle 必须是数组");
                }
                Some(Value::Array(arr)) => {
                    for (i, rule) in arr.iter().enumerate() {
                        if let Some(when) = rule.get("when").filter(|v| !v.is_null())
                            && let Err(msg) = check_formula(when) {
                                d.error(&format!("{path}.display.cellStyle.{i}.when"), "COLUMN_CELLSTYLE_WHEN_INVALID", msg);
                            }
                    }
                }
                _ => {}
    ...
    if let Some(edit) = column.get("edit").filter(|v| !v.is_null()) {
        if !edit.is_object() {
            d.error(&format!("{path}.edit"), "COLUMN_EDIT_INVALID", "edit 必须是对象");
        } else {
            if let Some(md) = edit.get("mode").and_then(|v| v.as_str())
                && !EDIT_MODES.contains(&md) {
                    d.warn(&format!("{path}.edit.mode"), "COLUMN_EDIT_MODE_UNKNOWN", format!("未知 edit.mode：{md}"));
                }
            for key in ["requiredWhen", "readonlyWhen", "validateWhen"] {
                if let Some(expr) = edit.get(key).filter(|v| !v.is_null())
                    && let Err(msg) = check_formula(expr) {
                        d.error(&format!("{path}.edit.{key}"), "COLUMN_EDIT_EXPR_INVALID", msg);
                    }
            }
        }
    }
...
fn validate_group_props(node: &Value, path: &str, d: &mut Diag) {
    if let Some(agg) = node.get("aggregate").filter(|v| !v.is_null()) {
        if !agg.is_object() {
            d.error(&format!("{path}.aggregate"), "GROUP_AGGREGATE_INVALID", "aggregate 必须是对象，如 { sum:true }");
        } else {
            for (k, v) in agg.as_object().unwrap() {
                if !GROUP_AGG_KEYS.contains(&k.as_str()) {
                    d.warn(&format!("{path}.aggregate.{k}"), "GROUP_AGGREGATE_KEY_UNKNOWN", format!("未知聚合类型：{k}（支持 sum/avg/max/min/count）"));
                } else if !v.is_boolean() {
                    d.error(&format!("{path}.aggregate.{k}"), "GROUP_AGGREGATE_VALUE_INVALID", format!("aggregate.{k} 必须是布尔值"));
                }
            }
        }
    }
    if let Some(pos) = node.get("aggregatePosition").and_then(|v| v.as_str())
        && !GROUP_AGG_POSITIONS.contains(&pos) {
            d.error(&format!("{path}.aggregatePosition"), "GROUP_AGGREGATE_POSITION_INVALID", format!("aggregatePosition 必须是 before/after，实际：{pos}"));
        }
}

fn validate_column_model_props(cm: Option<&Value>, path: &str, d: &mut Diag) {
    let Some(cm) = cm.filter(|v| !v.is_null()) else { return };
    if !cm.is_object() {
        d.error(path, "COLUMN_MODEL_OBJECT_REQUIRED", "columnModel 必须是对象");
        return;
    }
    for key in ["caption", "datasetId", "toTitleCols", "iconCol"] {
        if let Some(v) = cm.get(key).filter(|v| !v.is_null())
            && !v.is_string() {
                d.error(&format!("{path}.{key}"), "COLUMN_MODEL_FIELD_INVALID", format!("columnModel.{key} 必须是字符串"));
            }
    }
}
...
fn detect_formula_cycles(computed: &[Value], r_path: &str, d: &mut Diag) {
    ...
    fn visit(
        ...
        if visiting.contains(&id) {
            ...
            cycle.push(id.clone());
            d.error(&format!("{r_path}.detail.fields.{id}.dependsOn"), "FORMULA_DEPENDENCY_CYCLE", format!("计算公式存在循环依赖：{}", cycle.join(" -> ")));
            return;
        }
    ...
    for f in computed {
        visit(f, &by_code, &mut visiting, &mut visited, &mut stack, r_path, d);
    }
}
