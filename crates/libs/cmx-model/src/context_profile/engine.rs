...
fn field_display_name(field: &Value) -> String {
    for k in ["name", "label", "fieldName", "code"] {
        if let Some(s) = field.get(k).and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
            return s.to_string();
        }
    }
    field_id(field)
...
fn is_numeric_type(dt: &str) -> bool {
    matches!(dt.to_uppercase().as_str(), "INT" | "BIGINT" | "TINYINT" | "DECIMAL" | "NUMBER")
}

fn field_data_type(field: &Value) -> String {
    // 与 Node `_fieldDataType` 一致：field.dataType 为「真值」才用（空串视为缺省）。
    if let Some(dt) = field.get("dataType").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
        return dt.to_string();
    }
...
impl<'a> Engine<'a> {
    ...
    fn matched_rules(&self, anchor: &Map<String, Value>) -> Vec<(Value, f64, usize)> {
        ...
        out.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal).then(a.2.cmp(&b.2)));
        out
    }
    ...
    fn score_match(&self, match_obj: Option<&Value>, anchor: &Map<String, Value>) -> f64 {
        ...
        for (dim, cond) in m {
            ...
            let (base_dim, _attr_path) = dim.split_once('.').map(|(a, b)| (a, Some(b))).unwrap_or((dim.as_str(), None));
    ...
    fn score_condition(&self, dim_full: &str, base_dim: &str, code: &Value, cond: &Value) -> f64 {
        ...
        if let Some((_, attr)) = dim_full.split_once('.') {
            ...
            let attr_val = if attr == "code" { code.clone() } else { val.get(attr).cloned().unwrap_or(Value::Null) };
        ...
        match cond {
            ...
            Value::Null => 0.25,
            Value::Array(arr) => {
                if arr.iter().any(|v| same_value(code, v)) { 1.5 } else { -1.0 }
            }
            Value::Object(o) => {
            ...
            other => {
                if same_value(code, other) { 3.0 } else { -1.0 }
            }
        }
    }
    ...
    fn score_value_condition(&self, value: &Value, want: &Value) -> f64 {
        match want {
            Value::String(s) if s == "*" => 0.25,
            Value::Array(arr) => {
                if arr.iter().any(|v| same_value(value, v)) { 1.5 } else { -1.0 }
            }
            Value::Object(o) if o.keys().any(|k| k.starts_with('$')) => self.score_operator_condition(value, o),
            other => {
                if same_value(value, other) { 1.0 } else { -1.0 }
            }
        }
    }
    ...
    pub fn resolve_merged_rule(&self, anchor: &Map<String, Value>) -> Option<Value> {
        ...
        let mut by_code: std::collections::HashMap<String, (Value, f64, usize)> = std::collections::HashMap::new();
        let mut pos = 0usize;
        ...
        for (rule, _, _) in &by_def {
            if let Some(gs) = rule.get("detail").and_then(|d| d.get("groups")).and_then(|v| v.as_array()) {
        ...
        cm_order.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal).then(b.2.cmp(&a.2)));
        ...
        for (rule, _, _) in &by_def {
            if let Some(ds) = rule.get("anchor").and_then(|a| a.get("dimensions")).and_then(|v| v.as_array()) {
                for d in ds {
        ...
        let id = by_def.iter().filter_map(|m| m.0.get("id").and_then(|v| v.as_str())).collect::<Vec<_>>().join("+");
        let id = if id.is_empty() { "__merged__".to_string() } else { id };
        ...
        if !column_model.is_empty() {
            merged.as_object_mut().unwrap().insert("columnModel".to_string(), Value::Object(column_model));
        }
        Some(merged)
    }
    ...
    pub fn build_columns(&self, rule: &Value) -> Vec<Value> {
        rule.get("detail")
            ...
            .and_then(|v| v.as_array())
            .map(|fields| fields.iter().map(|f| self.field_to_column(f, rule)).collect())
            .unwrap_or_default()
    }

    /// _fieldToColumn + CmxColumn.toJSON 等价输出。
    fn field_to_column(&self, field: &Value, rule: &Value) -> Value {
        let req = field.get("edit").and_then(|e| e.get("required")).and_then(|v| v.as_bool()).unwrap_or(false);
        let id = field_id(field);
        let caption = field_caption(field);
        let dict_settings = self.dict_settings_for_field(field, rule);
        let base = field.get("column").and_then(|v| v.as_object()).cloned().unwrap_or_default();
        ...
        col.insert("caption".to_string(), json!(format!("{}{}", if req { "* " } else { "" }, caption)));
        let data_type = base.get("dataType").and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or_else(|| field_data_type(field));
        ...
        if col.get("integerDigits").map(|v| v.is_null()).unwrap_or(true)
            && let Some(l) = field.get("intDigits") {
                col.insert("integerDigits".to_string(), l.clone());
            }
        if col.get("decimalDigits").map(|v| v.is_null()).unwrap_or(true)
        ...
        if !display.contains_key("align") && field.get("dimType").and_then(|v| v.as_str()) == Some("measure") {
            display.insert("align".to_string(), json!("right"));
        }
        col.insert("display".to_string(), Value::Object(display));

        // edit = base.edit + field.edit
        let mut edit = base.get("edit").and_then(|v| v.as_object()).cloned().unwrap_or_default();
        ...
        if !edit.contains_key("mode") {
            let mode = if dict_settings.is_some() { "cmx-dict-selct".to_string() } else { self.col_edit_mode(field) };
            edit.insert("mode".to_string(), json!(mode));
        }
        if field.get("edit").and_then(|e| e.get("mode")).and_then(|v| v.as_str()) == Some("computed")
            && base.get("edit").and_then(|e| e.get("mode")).is_none()
        {
            edit.insert("mode".to_string(), json!("readonly"));
        }
        for (fk, ck) in [("requiredWhen", "requiredWhen"), ("readonlyWhen", "readonlyWhen"), ("placeholder", "placeholder")] {
        ...
        if !rules_v.is_empty() && !edit.contains_key("validate") {
            edit.insert("validate".to_string(), Value::Array(std::mem::take(&mut rules_v)));
        }
        ...
        if dict_settings.is_none() && !has_base_mode
            && let Some(enums) = field.get("enumValues").and_then(|v| v.as_array()).filter(|a| !a.is_empty()) {
        ...
        if field.get("dimType").and_then(|v| v.as_str()) == Some("dimension") {
            ...
            if dict_settings.is_none() && edit.get("mode").and_then(|v| v.as_str()) == Some("select")
        ...
        if field.get("dimType").and_then(|v| v.as_str()) == Some("measure") || is_numeric_type(&data_type) {
            let mut disp = json!({ "decimals": 2, "thousand": ",", "zeroBlank": true, "negativeColor": true });
    ...
    fn cmx_column_to_json(&self, props: Map<String, Value>) -> Value {
        ...
        let required = edit.get("required").cloned().filter(|v| !v.is_null()).or_else(|| get("required")).unwrap_or(json!(false));
        let display_mode = display.get("mode").and_then(|v| v.as_str()).filter(|s| !s.is_empty()).map(|s| s.to_string())
        ...
        json.insert("dataType".to_string(), get("dataType").unwrap_or(json!("VARCHAR")));
        json.insert("length".to_string(), get("length").unwrap_or(Value::Null));
        json.insert("integerDigits".to_string(), get("integerDigits").unwrap_or(Value::Null));
        json.insert("decimalDigits".to_string(), get("decimalDigits").unwrap_or(Value::Null));
        json.insert("calcFormula".to_string(), strip_fn(get("calcFormula")));
        json.insert("validateFormula".to_string(), strip_fn(get("validateFormula")));
        json.insert("displayMode".to_string(), json!(display_mode));
        json.insert("displayMask".to_string(), strip_fn(get("displayMask")));
        json.insert("actionRef".to_string(), get("actionRef").unwrap_or(Value::Null));
        ...
        // 透传额外键（非 KNOWN、非函数）
        const KNOWN: &[&str] = &[
            "id", "caption", "dataType", "length", "integerDigits", "decimalDigits", "calcFormula",
            "validateFormula", "displayMode", "displayMask", "label", "align", "actionRef", "width",
            "required", "visible", "frozen", "agg", "display", "edit",
            // 构造期消费但不输出的中间键
            "editSettings", "column", "type", "editMode",
        ];
        for (k, v) in &props {
    ...
    fn col_edit_mode(&self, field: &Value) -> String {
        let m = field.get("edit").and_then(|e| e.get("mode")).and_then(|v| v.as_str()).unwrap_or("cmx-text-input");
        match m {
    ...
    fn normalize_field_display(&self, base_display: Option<&Value>, field_display: Option<&Value>) -> Map<String, Value> {
        let mut src = Map::new();
    ...
    fn dict_settings_for_field(&self, field: &Value, rule: &Value) -> Option<Value> {
        ...
        let dict_code = field.get("refDict").and_then(|v| v.as_str()).map(|s| s.to_string())
        ...
        let id_col = dd("idCol").or_else(|| dd("idField")).and_then(|v| v.as_str().map(|s| s.to_string())).unwrap_or_else(|| "id".to_string());
        let code_col = field.get("refField").and_then(|v| v.as_str()).map(|s| s.to_string())
        ...
        let label_col = field.get("displayField").and_then(|v| v.as_str()).map(|s| s.to_string())
        ...
        let value_field = field.get("refField").and_then(|v| v.as_str()).map(|s| s.to_string())
            .or_else(|| dd("valueField").and_then(|v| v.as_str().map(|s| s.to_string())))
        ...
        let dict_title = dd("dictTitle").and_then(|v| v.as_str().map(|s| s.to_string()))
            .unwrap_or_else(|| format!("选择{}", if !cap.is_empty() { &cap } else if !dim_name.is_empty() { dim_name } else { &dict_code }));
        Some(json!({
            "dictCode": dict_code,
            "idCol": id_col,
            "codeCol": code_col,
    ...
    fn fields_depending_on_dimension(&self, rule: &Value, dim_code: &str) -> Vec<String> {
        let fields = rule.get("detail").and_then(|d| d.get("fields")).and_then(|v| v.as_array());
        let Some(fields) = fields else { return vec![] };
        let mut direct: Vec<String> = Vec::new();
        for f in fields {
            let dt = f.get("dimType").and_then(|v| v.as_str());
            let is_attr_src = dt == Some("attribute")
                && f.get("source").and_then(|s| s.get("dimension")).and_then(|v| v.as_str()) == Some(dim_code);
            let is_measure_df = dt == Some("measure")
                && f.get("defaultFrom").and_then(|s| s.get("dimension")).and_then(|v| v.as_str()) == Some(dim_code);
        ...
        for f in fields {
            ...
            if has_formula
                && let Some(deps) = f.get("dependsOn").and_then(|v| v.as_array())
                    && deps.iter().any(|d| d.as_str().map(|s| direct.iter().any(|x| x == s)).unwrap_or(false)) {
                        let id = field_id(f);
                        if !id.is_empty() {
                            downstream.push(id);
                        }
    ...
    pub fn build_members(&self, rule: &Value) -> Vec<Value> {
        ...
        let groups = rule.get("detail").and_then(|d| d.get("groups")).and_then(|v| v.as_array());
        let Some(groups) = groups.filter(|g| !g.is_empty()) else {
            return flat;
        };
        let by_id: std::collections::HashMap<String, Value> = flat
            .iter()
            .map(|c| (c.get("id").and_then(|v| v.as_str()).unwrap_or("").to_string(), c.clone()))
            .collect();
...
fn build_group_node(
    ...
    *counter += 1;
    let gid = props.get("id").and_then(|v| v.as_str()).map(|s| s.to_string())
        .or_else(|| props.get("caption").and_then(|v| v.as_str()).map(|s| s.to_string()))
        .unwrap_or_else(|| format!("__cp_g_{counter}"));
    let caption = props.get("caption").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    let aggregate_position = node.get("aggregatePosition").cloned().unwrap_or_else(|| json!("after"));
...
fn normalize_display(props: &Map<String, Value>) -> Map<String, Value> {
    let mut d = props.get("display").and_then(|v| v.as_object()).cloned().unwrap_or_default();
    ...
    if !d.contains_key("mode")
        && let Some(dm) = props.get("displayMode").and_then(|v| v.as_str()).filter(|s| *s != "text") {
...
fn normalize_edit(props: &Map<String, Value>) -> Map<String, Value> {
    let es = props.get("editSettings").and_then(|v| v.as_object()).cloned().unwrap_or_default();
    let mut e = es.clone();
