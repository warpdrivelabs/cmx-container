...
//! 五项元数据建模能力（BASE-DCT / BASE-DOC / DCT / DOC / CP）：
//! - [`definitions`]      —— DCT/DOC/BASE 定义中心（数据存 `data/meta/definitions/**`）。
//! - [`context_profile`]  —— 上下文档案 CP 规则引擎（数据存 `data/meta/context-profile`、`data/meta/subdivision`）。
//! - [`dict`]             —— 字典检索引擎（模型中心的字典能力，被 `context_profile` 依赖）。
//!
