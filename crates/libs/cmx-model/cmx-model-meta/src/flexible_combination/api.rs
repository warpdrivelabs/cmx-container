...
/// 收集字段列表中各字段引用的维度（dimension.refDict / source.dimension / defaultFrom.dimension）。
fn collect_field_dims(
    fields: Option<&Vec<Value>>,
    need: &mut std::collections::BTreeSet<String>,
) {
...
/// 读档案并做 dict-enrich + overlay 读时展开（与 `/rule` 的展开同源），
/// 供 `/config?expanded=1`（运行时 loadDefaultRule）消费。
///
/// # Arguments
///
/// * `r` - 弹性组合引用。
...
/// # Returns
///
/// 返回展开后的 combination；无 overlay 或 DOC 加载失败时与原文等价。
pub async fn config_expanded(r: &FcRef) -> PortalResult<Value> {
    let cfg = enrich_flexible_combination_dict_meta(&get_flexible_combination(r).await?).await?;
    Ok(expand_combination_overlay(&cfg).await)
}

/// 按 `dims` 从 `source` 收集锚点（仅非 null 值；标量归一为字符串，数组保持原样）。
///
/// 数组保持原样是为 `<dim>.__path` 祖先链（handler 拆 query 逗号串所得）——
/// 若被 `value_to_string` 归一成 JSON 串，引擎按逗号再拆会得到带引号/括号的碎片，
/// `$under` 比对将全部失败。
fn collect_anchor(source: &Map<String, Value>, dims: &[String]) -> Map<String, Value> {
    ...
    for d in dims {
        if let Some(v) = source.get(d).filter(|v| !v.is_null()) {
            let norm = if v.is_array() { v.clone() } else { json!(value_to_string(v)) };
            anchor.insert(d.clone(), norm);
        }
    }
    anchor
}
...
pub async fn rule(r: &FcRef, query: &Map<String, Value>) -> PortalResult<Value> {
    ...
    // need 集合：锚点维度 + 规则各字段集（detail.fields 与 fieldTabs[].fields）字段引用的维度
    // （refDict / source.dimension / defaultFrom.dimension）
    let mut need: std::collections::BTreeSet<String> = anchor_dims.iter().cloned().collect();
    collect_field_dims(rule.get("detail").and_then(|d| d.get("fields")).and_then(|v| v.as_array()), &mut need);
    if let Some(tabs) = rule
        .get("detail")
        .and_then(|d| d.get("fieldTabs"))
        .and_then(|v| v.as_array())
    {
        for t in tabs {
            collect_field_dims(t.get("fields").and_then(|v| v.as_array()), &mut need);
        }
    }
...
mod tests {
    ...
    /// `<dim>.__path` 数组（handler 拆 query 逗号串所得）必须保持数组形态进入引擎，
    /// 否则被 value_to_string 归一成 JSON 串后 $under 层级泛化比对全部失败。
    #[test]
    fn collect_anchor_keeps_array_path_for_under() {
        let mut src = serde_json::Map::new();
        src.insert("gl_account".to_string(), json!("222101"));
        src.insert(
        ...
        let anchor = collect_anchor(&src, &dims);
        assert_eq!(anchor["gl_account"], json!("222101"));
        assert_eq!(anchor["gl_account.__path"], json!(["2", "2221", "222101"]));

        // 引擎视角：数组 path 能被 $under 正确解析（串联验证链路末端）
        let rules = json!([
            { "id": "l1", "anchor": { "dimensions": ["gl_account"], "match": { "gl_account": { "$under": "2" } } },
              "detail": { "fields": [] } },
            { "id": "fb", "anchor": { "dimensions": ["gl_account"], "match": {} },
              "detail": { "fields": [] } },
        ]);
        let dims_def = json!({});
        let eng = crate::flexible_combination::engine::Engine::new(&dims_def, &rules);
        let merged = eng.resolve_merged_rule(&anchor).unwrap();
        assert_eq!(merged["id"], json!("l1+fb"));
    }
}
