...
/// 列出规则（rule_code + rule_name + mode + DAM 归属，供下拉源与管理页列表用）。DAM 非空时按模块过滤。
pub async fn list_rules(db_id: &str, dam: &Dam) -> Result<Vec<serde_json::Value>> {
    let mm = get_default_pg_db_manager();
    let (dam_clause, dam_params) = dam_where(dam, 1);
    let sql = format!(
        r#"SELECT rule_code, rule_name, mode, domain_code, application_code, module_code FROM cmx_code_rule
    ...
    for row in &ds.rows {
        ...
        let domain_code = dv_as_str(row.get(3)).unwrap_or("").to_string();
        let application_code = dv_as_str(row.get(4)).unwrap_or("").to_string();
        let module_code = dv_as_str(row.get(5)).unwrap_or("").to_string();
        result.push(serde_json::json!({
            "ruleCode": rule_code, "ruleName": rule_name, "mode": mode,
            "domainCode": domain_code, "applicationCode": application_code, "moduleCode": module_code,
        }));
    }
    Ok(result)
}
