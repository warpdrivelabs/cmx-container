-- =============================================
-- 迁移说明：模型中心·数据库初始化与模块部署台账（目标库内系统表 + 主控库汇总表），含完整表/字段注释
-- 影响表：cmx_model_meta, cmx_model_module, cmx_model_module_kind, cmx_model_deploy_history, cmx_model_source, cmx_model_registry
-- 操作类型：CREATE TABLE / CREATE INDEX / COMMENT
-- 回滚方式：20260702_001_model_center_ledger.down.sql
-- =============================================
-- 幂等：全部 CREATE TABLE IF NOT EXISTS / CREATE INDEX IF NOT EXISTS；COMMENT 为覆盖式，可重复执行。
-- 说明：目标业务库的这几张表由后端 PgTableDefineExecutor 编程建立（支持任意 db_id，
--       与 cmx-model-deploy::ledger::LEDGER_INIT_DDL 同源同注释）；
--       本迁移文件用于「默认/主控库」在启动时也拥有同名结构 + 主控库专属的 cmx_model_registry。
--       注意：已执行过的迁移不会因文件修改而重跑（cmx_schema_migrations 有记录即跳过）；
--       如需让存量库补上注释，可手工重跑本文件（语句全部幂等）。
...
-- =============================================
-- 2. 每模块当前态（工作台「已部署」来源；各 kind 状态按行存 cmx_model_module_kind）
-- =============================================
...
-- =============================================
-- 3. 模块类型当前态（每模块每 kind 一行；新增类型不改表结构）
-- =============================================
...
-- =============================================
-- 5. 源定义/初始数据 JSON 完整留档（每模块×kind×版本一行）
-- =============================================
