...
--
-- 三批内容合一（定稿前未提交，无中间版本）：
--   ① 编码规则 14 条：MDM_KH/WL/KJ（流水码）+ 10 个固定业务码域 ref 段（code=业务码列）+ MDM_GYS 兜底
--   ② 激活映射 26 条：10 新域 × create/update + kh/wl/kj 深化字段版 × create/update
--   ③ 查重规则 13 条：id 2~14，specs/cluster/survive 与元数据列对齐
--
-- 实体总览（元数据 data/meta/definitions/basic/dataplatform/mdm/dataplatform_dct_meta_v1.json，共 18 表）：
--   客商域 customer（头+bank/contact/address 三明细，MDM_KH=CUS+日期+流水）
--   物料域 material（纯头，MDM_WL=MAT+日期+流水）
--   财务域 gl_account（树形，MDM_KJ=ref(acct_no) 科目号原值铸 code）
--   档案域 currency/uom/material_class/cost_center/profit_center/company/organization/department/position/employee
--     （统一 ref 段铸号：MDM_BZ/MDM_JLDW/MDM_WLDL/MDM_CBZX/MDM_LRZX/MDM_GS/MDM_ZZ/MDM_BM/MDM_GW/MDM_YG）
--   MDM_GYS：supplier 域历史遗漏兜底（运行库已有 UI 配置则 DO NOTHING 不覆盖，仅新环境生效）
--
-- 跨字典依赖（元数据字段级 refDict 声明，激活映射直接搬运引用 id，cr-form 渲染 dict-select）：
--   currency ← company.base_currency_id / customer.settle_currency_id
--   material_class ← material.class_id；uom ← material.base/purchase/stock_uom_id
--   company ← organization.company_id / employee.company_id
--   organization ← department.org_id；department ← cost_center.dept_id / employee.dept_id
--   position ← employee.position_id；employee ← customer.customer_manager_id
--   树形自引用（gl_account/material_class/cost_center/profit_center/organization/department）：parent_id refDict=自身
--
-- 激活器 create 分支会用占位码覆盖 code，仅铸号成功可再覆盖——固定业务码实体走 ref 段
-- 「借铸号通道」把用户手填的业务码写入 code 列，无需改 Rust（见 MDM_KJ 注释）。
-- 建表/seed：模型中心 deploy DCT→SEED；菜单：mdm-menu.json + sync_menu_db.py 同步 cmx_menu。
-- =============================================
...
--      （§1 的 MDM_GYS 编码规则兜底同因）。字段对齐现行 supplier 字典：
