...
-- ============================================================
-- 出站 webhook 订阅 + 持久化投递队列（cmx-flowengine 001 方案 v2.3）
-- 主键 = BIGINT 应用层 Pk52 雪花（cmx-utils next_pk_id()，52 位 JS 安全）；
-- 投递表保序键独立为 seq BIGSERIAL（DB 提交序，多副本时钟偏移免疫）——
-- 身份与排序分离，对齐 mdm 分发订阅模块（md_subscription / md_dispatch_log）。
-- 引擎侧 biz_link 式幂等自举 DDL 同源维护（cmx-flow-app/src/webhook_store.rs）。
-- ============================================================

-- 订阅配置表（结构对齐 md_subscription）
...
-- 持久化投递队列 + 死信一体（结构对齐 md_dispatch_log；事务后持久化队列，非严格 outbox）
