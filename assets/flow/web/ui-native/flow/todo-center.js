...
function mount (ctx, view) {
  ...
  // 待办加载不能单押帧回调——rAF 与 setTimeout(0) 谁先跑谁置位，另一方跳过。
  let booted = false
  const boot = () => {
    if (booted) return
    booted = true
    render(); if (view === 'content' || view === 'explorer') loadTodos(); if (view === 'content') loadFilterOptions()
  }
...
function buildApprovalSteps ({ instance, definition, comments, task }) {
  ...
  const firstCommentAt = normalized[0]?.createdAt || ''
  const steps = nodes.map((node, index) => {
    ...
    const done = !current && (
      ...
      || isStart
      || (!isStart && !branch && downstreamHasEvidence(node.id))
    )
...
function propertyHtml () {
  ...
  return `<section class="todo todo-prop">
    ...
    <div class="todo-prop-body">
      ...
      <div class="todo-sec">流程进度</div><cmx-flow-trail></cmx-flow-trail>
    </div></section>`
}

// 待办中心只做轻量预览：事件流轨迹（<cmx-flow-trail> 组件，cmx-data-comp 同一份）；
// 完整意见与办理动作进入办理页。
...
function bind (root, view, host) {
  // 属性面板轨迹组件数据回填（cmx-data-comp 的 <cmx-flow-trail>，事件流口径）。
  root.querySelectorAll('cmx-flow-trail').forEach((el) => {
    el.trail = { instance: state.trail, definition: state.trailDefinition, comments: state.comments }
  })
...
function buildAndOpenTaskForm (t, f, sourceEl) {
  ...
  // console='none' 分支细化（20260902 方案修订：取消右侧独立属性面板）：
  //   - 表单页自带轨迹（portal.mdm.cr-form，轨迹+状态内嵌表单）→ 纯内容工作台标签（app.openNode），
  //     不叠 property，避免双轨迹；
  //   - 其它自足表单（页内无轨迹）→ 保持老链路 worknode + property 只读轨迹，保证任何待办都有轨迹可看。
  // 宿主异常找不到 portal-app 时同样兜底老链路。
  if (!usePlatformConsole && f.nativePage === 'portal.mdm.cr-form') {
    let app = null
    ...
    if (app && typeof app.openNode === 'function') {
      app.openNode({
        ...
        domain_code: taskCtx.domain, application_code: taskCtx.application,
        workspace: { id: `flow_task_${sid}`, params: taskCtx, content: contentRegion },
      }, { initialContext })
      return
    }
  }
...
function styleCss () {
  ...
  .todo-flow-kind{flex:0 0 auto;align-self:center;font-size:10px;font-weight:600;color:var(--muted);border:1px solid color-mix(in srgb,var(--muted) 45%,transparent);border-radius:999px;padding:1px 7px;white-space:nowrap}
