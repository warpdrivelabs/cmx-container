...
- **示例**: `"postgresql://postgres:postgres@192.168.1.100:5433/cmx"`
...
门户通知中心（任务/消息/日志三中心 + 群发，存储平台库 `cmx_notification` / `cmx_notification_recipient`）。
...
- **必需**: 否
- **默认值**: `90`
- **说明**: 未读通知保留天数。`publish` 未显式给 `expireAt` 时按此倒推过期时间；超期主体由清理任务分批删除
...
- **必需**: 否
- **默认值**: `2000`
- **说明**: 异步展开阈值。收件人数达到阈值时发布请求转后台异步展开（主体先落库 `pending` 立即返回，收件行渐进可见）
...
- **必需**: 否
- **默认值**: `30`
- **说明**: 每发布方每分钟发布上限（Redis 计数窗口 60s；用户身份按 user_id 计数，纯服务身份按 source 维度计数）。Redis 不可用时放行（fail-open），超限返回 429
