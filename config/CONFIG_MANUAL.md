...
- **必需**: 否
- **默认值**: `false`
- **说明**: 是否启用 RPC 功能。启用后，服务将启动 gRPC Server 并注册 RPC 客户端
- **示例**: `true`
...
- **必需**: 否
- **默认值**: `"grpc"`
- **说明**: RPC 协议类型。当前支持 `grpc`，未来可扩展 `http_rest`
- **示例**: `"grpc"`
...
- **必需**: 否
- **默认值**: `[]`
- **说明**: 启动时预热的服务名列表。首次 RPC 调用前预先拉取这些服务的实例缓存，避免首次调用延迟
- **示例**: `["cmx-order-service", "cmx-user-service"]`
...
- **必需**: 否
- **默认值**: `30`
- **说明**: 服务列表定时同步间隔（秒）。0 表示禁用定时同步。启用后，系统会定期从注册中心拉取服务名列表，发现新服务时自动建立实例订阅，确保 RPC 客户端能感知所有已注册服务
- **示例**: `30`
...
- **必需**: 否
- **默认值**: `9090`
- **说明**: gRPC Server 监听端口，与 HTTP 端口分离
- **示例**: `9090`
...
- **必需**: 否
- **默认值**: `4`
- **说明**: 连接池大小（HTTP/2 多路复用，通常 1-4 个连接即可）
- **示例**: `4`
...
- **必需**: 否
- **默认值**: `5000`
- **说明**: HTTP REST 调用超时时间（毫秒）（预留，本期不实现）
- **示例**: `5000`
